# Docker deployment candidate (Iteration 0.28.0 / v0.28.0 / Schema 11)

This directory is a fail-closed deployment skeleton, not a verified deployment recipe. Docker was unavailable on the development host, the image was not built, and Stage 0 has not supplied a reviewed egress host set. Current Schema 11 retains the Schema 10 separation of static implementations, immutable Stage 0 v3 evidence and append-only decisions, and adds the Windows local-app claim-gate singleton. None of this changes the unverified graph-v2 or target-Linux boundary. Do not mark any platform, the Docker candidate, or an X graph path `verified` from these files.

Iteration 0.13.0 adds the Schema 10 capability evidence/decision ledger, revision CAS and three-layer read API plus a single-transaction UI snapshot while retaining 0.12.0 ready-asset auxiliary delivery, TikTok short-link gate and offline fake E2E. Stage 0 `product_version` binds the release version to the importable package payload SHA-256 under the documented generated-cache exclusion: only `__pycache__/*.pyc` is excluded, other cache data remains covered, and links/reparse points/special files or inventory drift are rejected. A different build therefore cannot reuse approval merely by keeping `0.13.0`; historical rows remain readable and revocable. Imported CSV remains an operator assertion, not cryptographic execution attestation. This candidate still does not run real TikTok media, compose the short-link egress service, or prove target Linux behavior.

Iteration 0.14.0 reuses that resolver in the separate Windows local Worker and adds a no-claim `--check` mode. The shared resolver now rejects empty or oversized inputs and the same opened Cookie file being bound to different platforms. These changes do not make the Windows direct Worker part of this isolated Compose topology and do not replace the stricter Linux host ACL/mount acceptance contract.

Iteration 0.15.0 adds the Windows-only `video-download-local-app` supervisor, which binds the control plane and local Worker to one explicit application root/database/tool configuration and manages their startup and shutdown. It remains a direct, non-isolated host path and is not part of this four-service Linux candidate topology.

Iteration 0.16.0 adds Schema 11 and a two-phase, run-scoped claim gate for that Windows supervisor. It prepares the gate closed, activates it only after Worker preflight and a second control-plane identity check, and commits stop before closing the Worker command channel. The gate is scoped to a Worker launched with the matching local-app run ID; the candidate Linux Worker does not bind that run ID, so this change does not establish a Compose shutdown protocol or target-Linux acceptance. The same iteration adds explicit new-generation retry only for terminal failed flat downloads; retry does not bypass platform cooldown or a required manual circuit reset.

Iteration 0.17.0 adds a fixed, redacted yt-dlp stdout control protocol and a bounded line observer so the Windows direct Worker can persist monotonic stage estimates for download and post-processing. Subtitle and other sidecar output is ignored for progress; sequential main-media transfers and indexed fragments use conservative bounded aggregation, and unknown totals continue heartbeat without inventing a percentage. The Web/API render the persisted estimate. This does not alter the Compose network or credential topology, and the Linux acceptance runner still does not perform a real media download or validate these progress events on target Linux.
...
Iteration 0.5 wired admin-only Job assignment and claim-time `credential_profile_id` validation to the opaque `credential_ref` used by Adapter probe/download requests. The public Batch API rejects this field. Iteration 0.6 added Schema 8 parent discovery, immutable snapshots and per-attachment children, but that graph path has only been exercised with the offline `ScriptedGraphFakeAdapter`. Iteration 0.7 added a static, deployment-owned Cookie injection override, a default-read-only Linux/Docker acceptance runner and a separate, default-disabled short-link UDS transport; no Cookie file, Compose container or short-link egress process was exercised. Iteration 0.8.1 directly exercised the pinned Windows tools against one authorized YouTube URL and one authorized X URL. Iteration 0.9.0 then exercised those same exact samples through the host API queue, Windows Worker, real yt-dlp/FFmpeg/ffprobe, AssetStore/manifest and asset-download path. Iteration 0.10.0 expanded explicit six-platform routing while the database remained Schema 8. Those two live samples remain historical/narrow evidence, not platform-wide or deployment evidence. The Linux candidate still uses `YtDlpAdapter.supports_exact_selector=False` and cannot safely select one X attachment. The historical Iteration 0.5 / Schema 7 and Iteration 0.6–0.10 / Schema 8 records remain valid only for their recorded versions and scopes; they do not cover this deployment topology, the later Schema 9 identity, the Schema 10-introduced CSV v3 product-build identity and decision ledger, or the current Schema 11 claim-gate structure.

Iteration 0.18.0 gives the Windows local app and standalone local Worker's drain/poll modes two real execution slots, continuously refilled subject to the existing global 2 / platform 1 SQL caps. Live attempts stay excluded from recovery until their final cleanup returns. Shared subprocess interruption supports standalone Ctrl+C. The Linux candidate loop is unchanged; this is not target-Linux concurrency evidence.

Iteration 0.19.0 connects controlled in-process short-link expansion to Windows `video-download-local-app --allow-direct-network`, and adds explicit run-scoped Cookie defaults through v2 local-app configuration. Normal startup registers a missing profile or reuses a unique usable same-platform/same-ref profile; disabled/expired profiles are never revived. JSON/import new-task requests select `use_default` (default) or `anonymous`; failed retry without a body preserves its old binding, while an explicit mode changes it atomically with the new generation. Deduplication never overwrites existing live/ready work. This does not change the Linux candidate's source mapping, credential assignment or egress topology. The generic control entry remains default-off for short links and never implicitly enables the Windows direct path. This iteration uses offline/simulated-network tests only, not real platforms or Cookie login. See the [v0.19 engineering record](https://github.com/HedgehogsGX/Open-Flame/blob/7b48a9fe4dae09279a3e986642af68263386e796/validation/iteration-0.19.0-short-links-cookie-defaults-evidence.md).

Iteration 0.28.0 adds host-local editing, an optional separately built AI runtime, and a persistent URL workflow. None of those components is part of this Compose topology. The AI runtime uses a remote OpenAI provider when explicitly configured, while the current candidate egress allowlist and container isolation were not designed or accepted for that traffic. Workflow account `session_revision`, upload retry-leaf reconciliation, segment-local captions/dubbing, and local confirmation gates therefore do not establish target-Linux or container acceptance. No real OpenAI or Bilibili, Douyin, or WeChat Channels request was run for this candidate.

## Host-local state outside this topology (introduced in v0.9.0)
...
- For v0.28.0 ordinary Windows use, prefer `video-download-local-app`; it owns one canonical application root and derives one Schema 11 database for both child processes, verifies the control/Worker handshakes, prepares the claim gate closed, and activates it only for normal mode after preflight. `--check` never activates the gate or automatically registers default credential profiles. Normal v2 startup applies only an explicit `default_cookie_platforms` list referencing existing source entries (`platform`, `credential_ref`, `path`); v1 remains source-only. `GET /api/v1/credential-defaults` returns only configured platform names and local `available`, not profile IDs/refs/paths or proof of platform login. On shutdown it commits the gate stop before closing the Worker Pipe; failure to establish that SQLite order forces termination of the owned Job. Advanced separate control/Worker operation must still pass the same `--data-root` and explicit `--database-path` used by `VDC_DATA_ROOT`/`VDC_DATABASE_PATH`; creating a second database creates a different queue. One supervisor and one local Worker lock are allowed per application/data root. Earlier runs remain historical evidence only.
- Windows short-link requests share the resolver's 5-hop, 5-second per-request and 15-second chain limits, plus one 15-second budget across a Batch's unique short links. The in-process transport has 4 concurrent request slots and creates no listening socket or persistent event-loop thread; DNS has bounded slots but cannot forcibly cancel a blocked system resolver. These are separate from the media Worker's global 2 / platform 1 execution caps and do not provide namespace/process isolation. The full startup and v2 JSON examples are in the [Runbook](../docs/RUNBOOK.md).
...
- `GET /api/v1/batches/{batch_id}/assets` lists ready original metadata, its relative `download_url`, and registered thumbnail/caption DTOs in `artifacts`. `GET /api/v1/assets/{asset_id}/download` serves only the DB-registered ready original after data-root containment and plain/non-link/single-link checks. `GET /api/v1/artifacts/{artifact_id}/download` separately serves only a registered thumbnail/caption belonging to a ready asset/Job after canonical-ID, parent-original, path/name/MIME/language, identity, bounded-size and SHA-256 checks. The frontend renders both kinds of links with DOM APIs after the Batch becomes ready; neither response exposes a local path or uses a user title as the download filename.
...
- The exact YouTube and X samples completed Worker/AssetStore/manifest E2E, but both platform statuses remain `candidate` because sample coverage, Cookie cases, graph attachment selection, Linux isolation and Stage 0 are incomplete. The direct-tool evidence remains [Iteration 0.8.1](https://github.com/HedgehogsGX/Open-Flame/blob/7b48a9fe4dae09279a3e986642af68263386e796/validation/iteration-0.8.1-live-platform-evidence.md); the Worker E2E summary is [Iteration 0.9.0](https://github.com/HedgehogsGX/Open-Flame/blob/7b48a9fe4dae09279a3e986642af68263386e796/validation/iteration-0.9.0-local-worker-e2e-evidence.md).
...
- The candidate real Worker does not advertise exact-selector support. A Schema 11 graph `discover` or `x_attachment` Job fails closed at claim; operators must prevent such Jobs by keeping the host control plane's `VDC_ENABLE_X_GRAPH_V2=0`.
...
- Build-dependency installation, the project-wheel build, and runtime installation each use `RUN --network=none` and `--no-index`. The project is built with `--no-build-isolation --no-deps`, copied and installed by the exact path `/tmp/video_download_control-0.28.0-py3-none-any.whl`, and the runtime stage finishes with `python -m pip check`.
...
docker build \
  ...
  --tag registry.example/vdc-candidate:0.28.0 \
  .
```

Push the result to a private registry and use its resulting `name@sha256:...` reference in `VDC_CANDIDATE_IMAGE`. The Compose file uses `pull_policy: never`; preload that exact image on the single host before startup. The acceptance runner binds every direct helper and runtime container to the captured immutable image ID. Its network-none runtime-contract run checks the 14 locked runtime distributions plus `video-download-control==0.28.0`, requires application Schema 11, and rejects build-only distributions. These checks are implemented but have not been executed on target Linux; the image, tag and runtime package have not been built or exercised on the current Windows host.
...
Schema 11 preserves the Schema 8 graph model and the Schema 10 immutable Stage 0 v3 evidence, append-only decisions and read-only current view keyed by the complete route/version/environment identity. Its additional local-app claim-gate table does not add exact-selector support. Graph orchestration remains validated only with `ScriptedGraphFakeAdapter`.
...
[`run-linux-acceptance.sh`](run-linux-acceptance.sh) defaults to `--preflight`, which does not build, start/stop containers, create sockets, restore data or clean artifacts. Mutation mode additionally requires the exact `--execute --authorize I_ACCEPT_TARGET_LINUX_MUTATIONS` pair, an explicit protected env file, immutable tool image, Schema 11 backup root and empty restore parent.

The v0.17 runner accepts a previously audited Schema 11 backup for its retained-restore check. Do not feed it a Schema 8, 9 or 10 backup directly: restore with the matching historical application, migrate a copy with v0.17, validate readiness, and create a new audited Schema 11 backup for this acceptance run. This documented contract and the runner's static checks are not evidence that the restore or any other acceptance step has run on target Linux.
