# Docker deployment candidate (Iteration 0.27.0 / v0.27.0 / Schema 11)
...
- Build-dependency installation, the project-wheel build, and runtime installation each use `RUN --network=none` and `--no-index`. The project is built with `--no-build-isolation --no-deps`, copied and installed by the exact path `/tmp/video_download_control-0.27.0-py3-none-any.whl`, and the runtime stage finishes with `python -m pip check`.
...
docker build \
  ...
  --tag registry.example/vdc-candidate:0.27.0 \
  .
```

Push the result to a private registry and use its resulting `name@sha256:...` reference in `VDC_CANDIDATE_IMAGE`. The Compose file uses `pull_policy: never`; preload that exact image on the single host before startup. The acceptance runner binds every direct helper and runtime container to the captured immutable image ID. Its network-none runtime-contract run checks the 14 locked runtime distributions plus `video-download-control==0.27.0`, requires application Schema 11, and rejects build-only distributions. These checks are implemented but have not been executed on target Linux; the image, tag and runtime package have not been built or exercised on the current Windows host.
