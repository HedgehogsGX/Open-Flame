...
def test_candidate_dockerfile_pins_base_and_requires_digest_tool_bundle() -> None:
    ...
    assert "video_download_control-0.24.2-py3-none-any.whl" in dockerfile
    assert 'org.opencontainers.image.version="0.24.2"' in dockerfile
    ...
    assert (
        ...
        "/project-wheel/video_download_control-0.24.2-py3-none-any.whl "
        "/tmp/video_download_control-0.24.2-py3-none-any.whl"
    ) in dockerfile
    ...
    assert "/tmp/video_download_control-0.24.2-py3-none-any.whl" in project_install
...
def test_python_build_and_runtime_lock_files_are_exact_and_hashed() -> None:
    ...
    assert 'version = "0.24.2"' in pyproject
    ...
    assert '"video-download-control": "0.24.2"' in runtime_contract
