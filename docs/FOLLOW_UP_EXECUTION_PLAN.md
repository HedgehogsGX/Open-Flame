...
| K：T20 发布后开发 | 自动流程 outcome、逐操作授权/硬上限、远端调用账本、复用预设、多输出 fan-out、服务端执行预检、安全重启续跑、编辑式玻璃四页生产视觉、译文精确修订绑定、上传 attention 精确确认恢复及当前应用根运行时刷新 | outcome、authorization/预算、Schema 4 ledger、Workflow Schema 2、预设/API/浏览器、四项运行就绪度、零下游副作用预检、原始 queued 预授权续跑、真实域离线 smoke、2×3 故障恢复、上传确认恢复、当前应用根 Setup/Start 及 K5 当前 44/44 Chromium 矩阵已通过；当前制品及真实网络/模型仍待验收 | provider key、账号 session、授权与当前 runtime 检查均通过后，日常 workflow 可从 URL 开始；unknown 不会被静默重放 |
