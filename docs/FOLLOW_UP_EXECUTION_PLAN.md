...
架构精简按小切片推进：上传标题/标签/平台选项/目标 override/定时规则已收敛为公开纯 metadata 契约；Workflow profile normalize/canonical JSON/SHA、来源标题 concrete upload 与有序 outputs 也已迁入公开纯 codec。上传执行、备份、Workflow service 和 preset 不再依赖这两类 service 私有实现，canonical 字节、默认字段省略、账号/override 顺序与 legacy profile 行为均保持。下一切片按评审 S3 收敛 upload slot 与账号绑定身份规则，再决定是否拆分 LocalWorkflowAdapter。
