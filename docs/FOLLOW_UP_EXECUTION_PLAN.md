...
路线制定：2026-09-05；进度更新：2026-09-10。依据：[本轮完整 Debug 核验](../validation/full-debug-20260905.md)。原执行基线为 `f12749f1dc8d0a2ebbb805669004e69ce33666a7`、v0.24.2。当前开发版本为 v0.28.0：下载 Schema 11、编辑 Schema 4、上传 Schema 3、Workflow Schema 3、上传备份格式 2。T18 的分段/封面编辑保留为 0.27.0 历史阶段；T19 已接入隔离 AI runtime、自动听写、中文与 English 翻译、标准音色配音以及 URL→编辑→所选三平台上传的持久化编排。`0592b6f` 的 0.28.0 五件制品已有有效包外 receipt；当前发布后开发在 T20 已收敛无人值守结果语义、逐操作 AI 精确授权和输入/调用硬上限，并完成 Schema 4 脱敏远程调用账本及 unknown 人工 reconciliation。预设回填、三项 AI 摘要绑定以及普通源码 Setup 的可选 AI/上传 runtime 入口均已在禁网临时根验证；Workflow Schema 2 的多分段、多输出上传 fan-out 已完成本地 synthetic 故障恢复验证，生产 `/workflows` 页面也已接通完整视频默认、可选 1–10 段输入、完整预设回填、fan-out 状态、四项运行就绪度和三平台独立投稿参数。当前 `segments=[]` 在无 AI 时也会经 Editing 域生成一个可上传的完整视频，cover-only 同时生成完整视频与封面；EditingManager 与直接 EditingService 执行入口共用同一语义。服务端现于保存 workflow 和建立下载前两次核对下载 Worker、FFmpeg、AI authorization/音色、上传 runtime/调度器、冻结账号绑定及既有封面资产，首次失败保持 workflow/event/download 为零；原始预授权且可证明尚未 dispatch 的 queued AI/render/upload 已完成安全重启续跑，整流程取消也已用 revision-fenced 持久意图、最远下游停止、批量上传身份核对及保守远端 outcome 完成本地验证。重复 URL workflow 现在会原子观察并等待既有下载 owner，复用其 ready 资产并传播精确失败/取消终态。配音语速已从 recipe/API/两页/预设贯通到 renderer、provider `speed` 与调用指纹，且保持默认 `1.0` 的旧 SHA 兼容。K5 已把用户选定的方向 C“编辑式玻璃”落入四页生产视觉并完成当前禁止远端的 Chromium 44/44 QA；编辑草稿及自动流程新 AI-ready 草稿现已精确绑定所选 translation `revision_id`，legacy 草稿只在完整匹配唯一时自动恢复，歧义时失败关闭；上传 attention 恢复也已改为按当前 leaf 是否仍含 draft 精确恢复确认边界，不再跳过确认或阻塞已确认任务。当前实际应用根已在保留旧 runtime 与私有 SQLite 备份后完成 AI/上传 runtime 构建验证、Upload Schema 1→3 迁移和实际本地 Start 检查；AI provider 因没有密钥按预期保持 `provider_health_required`。Workflow Schema 3 进一步在下载 ready 时把来源标题、绑定 asset 与逐账号最终标题一次冻结；页面只记住成功应用或保存的预设 ID，并在预设、账号和 AI capability 初始读取成功且用户未编辑时恢复，因此保存一次完整配置后可只更换 URL。当前源码仍待新的 clean receipt 冻结；下一步是形成候选并在单独明确授权下进行真实 OpenAI、真人试听与三平台实测。每份 receipt 只证明其绑定的 clean commit，不能覆盖之后的开发提交。

架构精简按小切片推进：上传标题/标签/平台选项/目标 override/定时规则已收敛为公开纯 metadata 契约；Workflow profile normalize/canonical JSON/SHA、来源标题 concrete upload 与有序 outputs 也已迁入公开纯 codec；S3 进一步把 ordered upload targets、账号 binding、取消批次和 current retry leaf 身份收敛为上传域纯契约。上传执行、备份、Workflow service、preset 与 LocalWorkflowAdapter 不再维护这些重复字段定义；canonical 字节、默认字段省略、账号/override/slot 顺序、legacy 64-root inspect、30-slot 上限、事务取消与 request digest 权限边界均保持。S4a、S4b、S4c 已分别集中 AI、Upload 与 Edit snapshot 的分类/观察/状态应用；人工确认、显式重试、普通/重启推进和 ledger reconciliation 不再复制结果映射，未知或畸形状态、已 active 工作及 reason 漂移也不会先触发 mutation-capable confirm/retry。授权、自动 reason allowlist、restart barrier、CAS、稳定请求键、下游 checkpoint、账号事务检查与取消权限仍留在调用方。S5 已完成：same-handle verified media response 与 EditingManager 分别进入独立公开模块，Editing 不再反向导入顶层 API 私有类，HTTP 文件也不再拥有后台执行循环；Range/逐块复核/句柄关闭、service factory 注入、lease、stop、取消及恢复时序保持。S6 已完成：Editing/Upload 完全相同的四字段文件身份、plain-entry `lstat`、匹配打开、重复 `fstat` 和带一字节增长哨兵的有界 SHA-256 已迁入公开根模块；两域 unavailable/unsafe/changed/size 映射、大小限制、复制/发布/回滚、事务和 retained-handle TOCTOU 合同保持分开。S7 已完成：Workflow 上传账号/公共字段读取、saved override 与用户修改的逐账号合并、即时校验、封面输入/校验、AI recipe 输入和最终 payload 编排已拆成普通函数；source-title、完整视频/分段、三平台差异、错误/focus 顺序、轮询期间值/焦点/文本选区及 capability 驱动限制保持。进一步共享 capability 或校验只在能删除已证实重复且不损失即时反馈时继续。下一切片按 S8 收敛 HANDOFF/validation 当前状态叙述，并确认 CI 与本地 hook 共用同一提交范围分类器。WorkflowStore 或更短 handler 只在能够删除现有重复且故障恢复语义可逐项证明时再提取。
...
| T20（0.28.0 发布后开发） | 已区分投稿接收、草稿保存和合法混合 outcome；账号失效撤回同账号 queued 确认；成功响应后轮换浏览器幂等键；workflow reconciliation 有界退避；逐操作 AI authorization 与输入硬上限已完成；Editing Schema 4 已保存脱敏远程调用状态，unknown 只能经三项固定结论人工 reconciliation；普通 Start 已实现仅在操作者显式配置时由 control child 继承精确的 OpenAI 密钥变量，本轮未配置密钥；普通源码 Setup 已可选构建/复用 AI runtime，并复用既有上传安装器准备 upload runtime；`/workflows` 已在创建前显示四项即时状态，服务端在写 workflow 与建下载前用真实域合同复核并冻结账号 session revision；原始预授权 queued AI/render/upload 可安全重启续跑；整流程取消会持久停止意图、按最远下游调用域取消，对 checkpoint gap 以稳定请求键发现，并核对 Editing 配方与 Upload v2 完整发布参数摘要；pending upload slot 的成功证据保持人工核对，running/unknown/部分远端成功同样保守等待或核对；当前实际应用根的 AI/上传 runtime 已验证，上传数据库已由 Schema 1 迁移到 Schema 3 并保留原记录 | 预设边界/API/生产页面浏览器、服务端零副作用预检、重启续跑、整流程取消、真实域离线整链、两个 Setup runtime 入口及当前应用根实际 Start 已通过；AI provider 因无密钥保持 `provider_health_required`，尚无真实 OpenAI、真人试听或平台接受证据；`request_units` 是本地 envelope 估算，不是价格、精确 HTTP 数或 usage receipt |
