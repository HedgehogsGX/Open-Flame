...
架构精简按小切片推进：上传标题/标签/平台选项/目标 override/定时规则现已收敛为公开纯 metadata 契约，上传执行、备份和 Workflow 预设不再依赖 Upload service 私有实现。下一切片迁出 Workflow profile codec，并保持 canonical JSON/SHA、默认字段省略、账号/override 顺序与 legacy profile 行为。
