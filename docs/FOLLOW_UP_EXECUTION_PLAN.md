...
路线制定：2026-09-05；进度更新：2026-09-07。依据：[本轮完整 Debug 核验](../validation/full-debug-20260905.md)。原执行基线为 `f12749f1dc8d0a2ebbb805669004e69ce33666a7`、v0.24.2，下载 Schema 11、上传 Schema 1；v0.24.4 的 T10 与 v0.25.0 的 T16/G6 保留为历史阶段，不能借给当前构建。当前开发版本为 v0.26.0，下载 Schema 11、上传 Schema 3、上传备份格式 2：T17 已完成首批三平台独立投稿参数、受管封面、定时值及本地 G7 回归与证据；下一门槛是最终冻结和真实平台验收。包内文件不预写自身 commit、hash 或完整 product identity；最终 clean commit、五件制品与独立安装是否完成只由同批包外 release receipt 判定。

## 1. 目标、边界与完成定义

下一阶段目标是让 **Bilibili、抖音、视频号** 上传具备可靠的失败处理、可解释的运行状态、可恢复的数据、逐平台参数和端到端证据，再形成外部测试及发行闭环。小红书等其他上传平台后续维护；本阶段不同时扩展平台数量、媒体剪辑或多 P/合集。
...
| T16 / G6（0.25.0 历史阶段） | T16.1～T16.7 本地开发与禁止远端调用的 G6 验收完成 | 保留为前端基线，不能证明 0.26.0 制品或平台能力 |
| T17（0.26.0 当前阶段） | 三平台独立标题/简介/标签、受管封面、定时和平台专属字段及本地 G7 已完成 | 真实平台逐字段接受、定时触发、封面裁切和发布结果 **NOT RUN**；最终制品只由包外 receipt 判定 |
| T11 / T12 | **NOT RUN** | 三平台真实上传与当前六平台下载必须绑定最终 0.26.0 receipt 后的同一构建分别执行 |
...
0.24.4、0.25.0 T16 与当前 0.26.0 T17 本地验收均未进行真实登录、扫码、平台下载或媒体上传。T11、T12、T15 与 GitHub hosted CI 当前均未运行；后续真实动作仍按[上传测试计划](UPLOADER_TEST_PLAN.md)逐项由测试员决定并执行。G6/G7 只证明各自本地禁止远端的工程范围，不能替代这些外部、平台或目标环境证据。
...
F. 0.26.0 最终冻结与包外 release receipt（由 receipt 判定）
           ↓ 有效 receipt：clean commit、五件制品、独立安装和冻结结果同一身份
G. 三平台上传与下载实测（T11、T12；当前 NOT RUN）
           ↓ G4：按平台、入口、模式分别接受
H. 根据真实回报修复并收敛当前功能（T13）
...
T16 不等待 T13 中无限期的更多平台/EXE 等功能扩展，也不等待暂未具备环境的 T15；其 0.25.0 本地实现依赖的 Windows 关键缺陷处置与 T04/T05/T14 界面状态契约已经收敛。T17 继续使用该前端规范并保持逐任务确认。T11/T12/T15 仍分别保持 **NOT RUN**，GitHub hosted CI 也仍为 **NOT RUN**；不能用 G6/G7 的本地页面结果、合成 backend 或视觉截图替代其中任何一项。0.24.4 的 T10 和 0.25.0 的 T16 是历史阶段；必须以 0.26.0 重新取得 clean commit、构建并独立安装五件制品，再由包外 receipt 绑定，才可交给 T11/T12 或称为最终交付包。
...
| H：0.26.0 最终冻结 | 发布维护与独立验收 | 由包外 receipt 记录 | clean commit、五件制品、独立安装和包外 receipt 同一身份 |

T15 的环境准备与 Linux/NAS 适配另估，不包含在 Windows 支线的工作量中。

当前执行顺序以 **0.26.0 包外 release receipt** 为门槛：没有有效 receipt 时，从最终源码取得 clean commit，再执行冻结全量、source snapshot、detached 构建及源码/wheel 两类独立验收；receipt 实际绑定同一提交、身份和五件制品后，才把该构建交给 T11/T12。T11、T12、T15 与 GitHub hosted CI 均仍为 **NOT RUN**，真实回报之后才进入 T13；这些状态不能由 0.24.4/0.25.0 历史结果或 0.26.0 本地合成验收改写。
