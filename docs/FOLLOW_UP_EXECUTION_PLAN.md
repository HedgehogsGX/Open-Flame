...
2026-09-14 当前入口：`9822454` 的 push run `34771038229` 四格通过，PR run `34771040471`
有一项封面测试等待竞争失败；固定合成存储负载 600.77 秒/300 轮及正常停机通过，见
...
[容量记录](../validation/iteration-0.28.0-upload-storage-threshold.md)；重启观察到的上传页连接错误
与旧会话已[独立修复](../validation/iteration-0.28.0-upload-poll-reconnection.md)。
下一步处理已复现的单行测试等待修正，再核对新提交 CI、继续其余恢复与真实业务验收。
...
22. **Workflow 投稿失败批量重试已接线。** 完整 fan-out 处于 `upload_job_failed` 时，Workflow 先观察并 checkpoint 当前 retry leaf，再由当时的 Upload Schema 3 单一 `BEGIN IMMEDIATE` 事务按稳定 request key/digest、完整 root payload、唯一 lineage、账号 session、媒体、封面、schedule 和当前平台合同核对全部 slot。该历史切片只为 failed/canceled leaf 创建新 draft；submitted、draft_saved、queued、running 和原 draft 原位保留，任一 unknown 整批阻断。响应丢失重放返回既有 leaf，部分准备不能进入 retry。所有新 draft 均再次要求明确确认；若同批另有原 draft，独立 `upload_retry_mixed_confirmation_required` 会说明最终确认范围。生产页面提供 `/uploads` 核对入口，完整失败批次才显示重试按钮。该历史切片本身没有新增表或 Schema；当前 Upload Schema 4 会在建草稿前对每个 failed/canceled leaf 执行完整 attempt receipt 校验，并以固定人工结论阻断 unknown 直接重传。见[Workflow 投稿重试验证](../validation/iteration-0.28.0-workflow-upload-retry.md)。

23. **Upload Schema 4 attempt receipt 与固定人工核对已接线。** 每个已领取 job 只有一个 receipt；领取事务原子写入 `reserved` 与 `running`，适配器调用前持久化 `dispatch_may_have_started`，返回结果与 job 终态同事务提交。receipt 绑定 request/job 摘要、账号 session revision、source/cover SHA-256、product build、追加式允许表中的 adapter identity、固定 evidence 和 revision。自动成功只接受四种平台/模式/evidence 精确组合。`unknown` 不再直接重试；必须读取 receipt、到对应平台后台核对并勾选确认，再以 CAS 提交固定结论，且只有 `not_accepted` 允许后续建立新草稿。没有 receipt 的 job 一律不能 retry，包括 `canceled / canceled`；可变 job 状态不能证明它从未 dispatch。备份格式 3 保存并语义审计 receipt；格式 1 / Schema 2 与格式 2 / Schema 3 仅作只读输入且不补造旧 receipt。当前实际 app root 尚未执行 Schema 4 迁移；本地 receipt 也不是平台签名证明。见[Upload Schema 4 attempt receipt 记录](../validation/iteration-0.28.0-upload-attempt-receipts.md)。
...
**当前接续（2026-09-14）：** `7575773` 已取得同提交工程 receipt，`1540a7e` 的 push/PR 四格也已通过；以上旧 CI 合同漂移已有修复。隔离普通启停/四页浏览器与旧 Upload 数据副本迁移、独立恢复已通过限定检查，见[隔离验收记录](../validation/iteration-0.28.0-release-readiness-drill.md)。最新核对的 `9822454` 固定存储负载 300 轮通过，push CI 四格通过，PR CI 的视频号封面测试等待竞争仍待限定维护，见[本轮记录](../validation/iteration-0.28.0-storage-soak-and-ci-follow-up.md)。main 门禁、全应用保护、实际根升级、其余故障/负载矩阵、真实模型和平台验收仍待完成；最终发行候选变更后重新完成受影响验收与制品冻结。
