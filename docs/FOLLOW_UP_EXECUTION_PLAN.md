...
路线制定：2026-09-05；进度更新：2026-09-07。依据：[本轮完整 Debug 核验](../validation/full-debug-20260905.md)。原执行基线为 `f12749f1dc8d0a2ebbb805669004e69ce33666a7`、v0.24.2，下载 Schema 11、上传 Schema 1；当前 T10 外测冻结源为 v0.24.4、下载 Schema 11、上传 Schema 2，精确 commit 与制品结果由包外 release receipt 记录。
...
本文件保留完整路线及验收门槛。2026-09-05 用户要求修复八项发现后，0.24.3 已实施 T01～T06 所对应的缺陷修复，以及 T07 的“已有上传库结构校验”部分；其审查范围见 [最终源码审查](../validation/iteration-0.24.3-final-review.md)。2026-09-07 的 0.24.4 冻结源继续完成 T14 必要切片、T07 停机备份/新根恢复、T08 有界 synthetic 韧性切片，以及 T09 工作流与本地 CI 合同。细节见 [0.24.4 本地工程证据](../validation/iteration-0.24.4-upload-data-lifecycle-evidence.md)。

| 工作包 | 当前可确认状态 | 后续门槛 |
| --- | --- | --- |
| T14 | 必要生命周期切片在本地完成 | 去重、总配额、自动孤儿清理仍为后续优化 |
| T07 | Upload Schema 2 的独立停机备份/恢复和合成演练在本地完成 | 真实容量、异机/offsite、NAS 与人工值班演练未做 |
...
| T10 | 版本号、第 14 个 wheel 命令入口及源码侧发行门禁已接线 | clean commit、五个制品与独立安装的实际结果必须由包外 release receipt 共同证明 |

冻结前本地定向结果不能单独替代 T10：上传相关精确集合为 **383 passed in 100.29s**；activity lock/上传备份/CLI 为 **133 passed in 49.68s**；包含 Windows 发布离线门禁的下载备份/发行/CI/验证/部署/API 组合为 **287 passed、4 skipped in 44.13s**，其中 4 个 skip 是 Windows 上的 root/POSIX/getfacl 环境合同。冻结 commit 的全量结果和源码/制品身份记录在包外 release receipt。

0.24.4 未进行真实登录、扫码、下载或媒体上传；后续真实动作仍按[上传测试计划](UPLOADER_TEST_PLAN.md)逐项由测试员决定并执行。新增生命周期控件已采用既有 Apple 设计基础，整站 Apple 风格升级继续排在最后 T16。
...
### T10 / P1 交付门槛：冻结并交付 0.24.4 外部测试包
...
执行顺序仍以 **T10 外部回执**为界：没有有效回执时，先完成冻结全量、source snapshot、clean commit 构建与两类独立验收；持有回执后，才把同一构建交给 T11/T12。真实回报进入 T13，整站视觉迁移仍按用户要求在最后的 T16 阶段完成并重新冻结最终制品。T15 保持独立环境支线。
