...
5. **非密钥复用预设已接线；下一步是本地整链 smoke。** `WorkflowPresetStore` 与 `/api/v1/workflows/presets` 已保存 URL 之外可复用的分段/封面/语言/音色/平台内容参数、authorization 摘要及账号选择意图；不得保存 API key、Cookie、扫码状态或账号 session revision，运行时重新绑定当前能力和账号。下一步执行一个禁止真实网络的 URL→下载→AI→编辑→Bilibili/抖音/视频号上传草稿 synthetic smoke，核对重启、unknown 与重复提交边界。
