...
路线制定：2026-09-05；进度更新：2026-09-07。依据：[本轮完整 Debug 核验](../validation/full-debug-20260905.md)。原执行基线为 `f12749f1dc8d0a2ebbb805669004e69ce33666a7`、v0.24.2。当前开发版本为 v0.27.0：下载 Schema 11、编辑 Schema 1、上传 Schema 3、上传备份格式 2。T17 的三平台参数保留为 0.26.0 历史阶段；T18 已完成非破坏性编辑工作台、分段、封面与下载→编辑→上传本地链路。下一开发门槛是 T19 隔离 AI 运行时、字幕导入/听写、中文与 English 翻译、标准音色配音及试听核对。包内文件不预写自身 commit、hash 或完整 product identity；最终 clean commit、五件制品与独立安装是否完成只由同批包外 release receipt 判定。

## 1. 目标、边界与完成定义

下一阶段目标是在不改变首批 **Bilibili、抖音、视频号** 上传边界的前提下，完成可核对的中间编辑流程，并继续接入隔离、本地优先的 AI 听写、翻译和标准音色配音。小红书等其他上传平台、多 P/合集与声音克隆后续维护。
...
| T18 / G8（0.27.0 当前阶段） | 独立编辑 Schema 1、版本化草稿、多个分段、封面、确认/取消/重试、真实本地 FFmpeg 与显式导入上传完成 | 编辑备份/恢复、长片/大文件矩阵及最终制品仍待后续；AI 处理保持 blocked |
| T19（下一开发阶段） | provider、capability 与 SRT/VTT 时间轴合同已建立 | 隔离 AI runtime、模型/许可证清单、字幕导入/ASR、中文与 English 翻译、标准音色配音、逐段试听和时长溢出处理未实现 |
| T11 / T12 | **NOT RUN** | 三平台真实上传与当前六平台下载必须绑定最终 0.27.0 receipt 后的同一构建分别执行 |
...
0.24.4、0.25.0 T16、0.26.0 T17 与当前 0.27.0 T18 本地验收均未进行真实登录、扫码、平台下载或媒体上传。T18 的真实媒体处理只使用本机生成的 synthetic 视频。T11、T12、T15 与 GitHub hosted CI 当前均未运行；后续真实动作仍按[上传测试计划](UPLOADER_TEST_PLAN.md)逐项由测试员决定并执行。
...
G. 隔离 AI 运行时、字幕/听写、翻译、配音与试听（T19）
           ↓ G9：固定依赖/模型、许可、隐私和短片人工试听通过
H. 0.27.0 最终冻结与包外 release receipt（由 receipt 判定）
           ↓ 有效 receipt：clean commit、五件制品、独立安装和冻结结果同一身份
I. 三平台上传与下载实测（T11、T12；当前 NOT RUN）
           ↓ G4：按平台、入口、模式分别接受
J. 根据真实回报修复并收敛当前功能（T13）
...
T18 延续 T16 设计规范和 T17 逐任务确认，不等待真实平台验收；T19 也不得绕过编辑成品复核或上传逐项确认。T11/T12/T15 与 GitHub hosted CI 仍保持 **NOT RUN**。不能用 G6～G8 的本地页面、合成媒体或截图替代外部能力证据。必须以 0.27.0 的 clean commit 构建并独立安装五件制品，再由包外 receipt 绑定，才可交给 T11/T12 或称为最终交付包。
...
### T18 / 当前阶段：非破坏性编辑工作台、分段与封面

**2026-09-07 当前状态：** v0.27.0 已完成本地实现、定向回归和真实 Chromium synthetic 流程。完整证据见 [0.27.0 编辑工作台记录](../validation/iteration-0.27.0-editing-workspace-evidence.md)。

- 编辑数据位于下载根同级的独立 `data-edits`，使用 Editing Schema 1。导入重新计算并核对下载成品 SHA-256，只在编辑域创建副本，不覆盖下载原件。
...
### T19 / 下一阶段：AI 字幕、翻译、配音与试听

1. 建立与核心 `.venv`、上传 runtime 分离的 `ai-runtime`；固定 Python/wheel/model revision、文件 SHA-256、许可证和可再分发状态。
2. 先支持导入已有 SRT/VTT；无字幕或用户明确重识别时才调用本地 faster-whisper，并把词级时间戳转换为统一毫秒时间轴。
3. 第一批翻译只开放中文与 English，保持 segment ID、顺序、来源文字、译文、provider/model revision 和人工核对状态。
4. 第一批配音只用标准音色，禁用声音克隆；逐段生成 WAV、测量实际时长，短段补静音，超长标记并要求修改或重新生成，不静默截断。
5. 提供逐段原文/译文/配音试听、重新生成、批准状态；全部通过后才能生成带配音视频。上传端仍保持现有逐项确认。
6. 本地模式、只发送文字的混合模式和完整媒体云模式分别显示外发数据、provider、估算费用和保留/删除规则；默认先完成全部本地路径。

**计划门槛 G9：** 全新 Windows 环境可以按 manifest 安装和验证 AI runtime；短片的字幕、翻译、标准音色、时长溢出、取消/恢复、模型缺失、无网络与损坏输出均有自动化和人工试听证据。任何云 provider 必须另有凭据存储、费用、隐私、远程 job 对账和删除状态验收。
...
| I：T19 | AI runtime、字幕/翻译/TTS 与试听 | 下一阶段 | G9 固定模型、许可/隐私、失败边界与人工试听 |
| J：0.27.0 最终冻结 | 发布维护与独立验收 | 由包外 receipt 记录 | clean commit、五件制品、独立安装和包外 receipt 同一身份 |

T15 的环境准备与 Linux/NAS 适配另估，不包含在 Windows 支线的工作量中。

当前最终发行顺序以 **0.27.0 包外 release receipt** 为门槛：从最终源码取得 clean commit，再执行冻结全量、source snapshot、detached 构建及源码/wheel 两类独立验收；receipt 实际绑定同一提交、身份和五件制品后，才把该构建交给 T11/T12。T19 可以在此前继续开发，但完成后须产生新的版本与独立证据，不能沿用 0.27.0 T18 的结果。T11、T12、T15 与 GitHub hosted CI 均仍为 **NOT RUN**。
