...
架构精简按小切片推进：上传标题/标签/平台选项/目标 override/定时规则已收敛为公开纯 metadata 契约；Workflow profile normalize/canonical JSON/SHA、来源标题 concrete upload 与有序 outputs 也已迁入公开纯 codec；S3 进一步把 ordered upload targets、账号 binding、取消批次和 current retry leaf 身份收敛为上传域纯契约。上传执行、备份、Workflow service、preset 与 LocalWorkflowAdapter 不再维护这些重复字段定义；canonical 字节、默认字段省略、账号/override/slot 顺序、legacy 64-root inspect、30-slot 上限、事务取消与 request digest 权限边界均保持。S4a 已集中 AI snapshot 在人工确认、普通/重启推进与 ledger reconciliation 三条路径的分类和状态应用，并修复人工确认未知 status 时的二次 adapter 调用；S4b 已集中四条 UploadSnapshot 路径的观察/分类与 retry-leaf checkpoint 顺序，修复非法/已确认观察仍调用上传确认的副作用。授权、双 reason allowlist、单轮 restart barrier、CAS、ledger 异常、账号事务检查与取消边界仍留在调用方。下一切片 S4c 评估 EditSnapshot 的重复结果解释；再根据实测收益决定是否拆分 LocalWorkflowAdapter。
