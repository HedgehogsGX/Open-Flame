...
默认导入上传视频或封面后保留 64 MiB 可用空间。需要提高预留量时，在源码根目录执行：

```powershell
.\Start-Open-Flame.cmd --upload-storage-min-free-bytes 536870912
```

此例为 512 MiB；参数接受 0 到 `9223372036854775807` 的整数字节数。
上传页“上传媒体占用”显示实际可用空间和本次启动的预留值。空间不足时，导入或恢复
返回 `upload_storage_full`；先核对容量与受管文件，再自行决定释放哪些空间。

把参数保存在实际使用的启动命令或本地快捷方式中，每次启动继续提供。它是本机部署配置，
不写进任务、预设或上传备份；恢复到另一机器时按该机器的启动配置生效，旧备份不会覆盖它。
普通 Start 通过此 CLI 参数传给控制子进程；独立 `video-download-control` 入口则读取
`VDC_UPLOAD_STORAGE_MIN_FREE_BYTES`，两种入口均不会自动载入 `.env` 文件。

阈值只约束上传领域的新媒体写入，包括接收临时文件、受管复制和媒体恢复。
HTTP 导入同时检查临时接收与受管副本所需的两份空间；复制期间及登记前继续复核。
下载保留自己的阈值，两域不会把这个数值相加作为磁盘配额。其他进程仍可能同时占用磁盘，
这些检查没有全卷原子空间预订；系统实际满盘时会清理本次未完成副本并报告相同错误。
上传数据库与备份结构不变；整份备份创建/恢复仍按其独立维护流程校验目标空间。
