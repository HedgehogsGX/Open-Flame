...
| WF-07 | 自动流程 | 保存/恢复“优先使用下载到的来源封面”，再关闭“生成封面” | 显式偏好精确恢复；生成封面始终作为 fallback，关闭它会同时清除来源封面偏好。详见[来源封面验证](https://github.com/HedgehogsGX/Open-Flame/blob/7b48a9fe4dae09279a3e986642af68263386e796/validation/iteration-0.28.0-workflow-source-cover-preference.md)。 |
...
来源封面测试须保留生成封面 fallback，并用 frozen `download_asset_id`、最终 `upload_cover_id` 和平台后台事实核对；完整矩阵见[来源封面验证](https://github.com/HedgehogsGX/Open-Flame/blob/7b48a9fe4dae09279a3e986642af68263386e796/validation/iteration-0.28.0-workflow-source-cover-preference.md)。
...
| E2E-13 | 来源封面偏好：覆盖可信 owner 下的 0/1/多候选、登记伪装/漂移、三平台共同兼容、3 分段、选择后/导入后/建 jobs 后崩溃重启、封面媒体删除、取消 tombstone 重放/篡改 | 唯一且全体兼容时采用来源封面；0 个或不兼容时回退生成封面；歧义/完整性漂移停下；任何 Upload 写入前先 CAS 冻结 cover ID。已有请求从不可变历史恢复且不要求终态封面字节；无请求的取消先原子 tombstone，重启不依赖 Download，并继续取消前序分段。详见[来源封面验证](https://github.com/HedgehogsGX/Open-Flame/blob/7b48a9fe4dae09279a3e986642af68263386e796/validation/iteration-0.28.0-workflow-source-cover-preference.md)。 |
...
2026-09-13 维护者已按用户授权应用固定差分的测试合同维护及后续平台 fixture 维护；
具体提交为 `3e482f0` / `ac3532b`，这不是外部测试者的通用例外。
具体范围及后续 CI 见 [CI 合同维护记录](https://github.com/HedgehogsGX/Open-Flame/blob/7b48a9fe4dae09279a3e986642af68263386e796/validation/iteration-0.28.0-ci-contract-maintenance.md)，
其他测试修改仍须单独获得对应授权。
