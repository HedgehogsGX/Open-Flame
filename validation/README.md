...
Current iteration: 0.28.0 post-release workflow development. The latest bounded result is [Workflow recipe function evidence](iteration-0.28.0-workflow-recipe-functions.md), followed by [Workflow upload form function evidence](iteration-0.28.0-workflow-upload-form-functions.md), [managed-file read boundary evidence](iteration-0.28.0-managed-file-read.md), [managed-file identity boundary evidence](iteration-0.28.0-managed-file-identity.md), [EditingManager boundary evidence](iteration-0.28.0-editing-manager-boundary.md), [verified media response boundary evidence](iteration-0.28.0-verified-media-response.md) and [EditSnapshot observation evidence](iteration-0.28.0-edit-snapshot-observation.md) and [UploadSnapshot observation evidence](iteration-0.28.0-upload-snapshot-observation.md), [AI snapshot application evidence](iteration-0.28.0-ai-snapshot-application.md), [public upload identity contract evidence](iteration-0.28.0-upload-identity-contract.md), [public Workflow profile contract evidence](iteration-0.28.0-workflow-profile-contract.md), [public upload metadata contract evidence](iteration-0.28.0-upload-metadata-contract.md), [no-AI whole-video and cover-only evidence](iteration-0.28.0-no-ai-full-video.md), [hosted CI execution-chain recovery evidence](iteration-0.28.0-hosted-ci-recovery.md) and [download HTTP boundary evidence](iteration-0.28.0-download-http-boundary.md) and [Workflow scan recovery evidence](iteration-0.28.0-workflow-manager-recovery.md) and [workflow source-title snapshot evidence](iteration-0.28.0-workflow-source-title.md). External validation starts from the repository [testing manual](../TESTING.md); use the [Debug guide](../docs/DEBUG_GUIDE.md) to isolate failures and return findings through the [external tester handoff template](../docs/EXTERNAL_TESTER_HANDOFF_TEMPLATE.md). This follows the [current application-root runtime refresh](iteration-0.28.0-local-runtime-refresh.md), [workflow upload attention recovery](iteration-0.28.0-workflow-upload-attention-recovery.md), [exact translation revision binding](iteration-0.28.0-translation-revision-binding.md), [editorial glass production frontend](iteration-0.28.0-editorial-glass-frontend.md), [speech-rate propagation](iteration-0.28.0-speech-rate.md), [duplicate-download owner recovery](iteration-0.28.0-workflow-duplicate-download-owner.md), [whole-workflow cancellation](iteration-0.28.0-workflow-cancellation.md) and historical [0.27.0 non-destructive editing workspace, video segments and cover generation](iteration-0.27.0-editing-workspace-evidence.md), [0.26.0 upload parameters, managed covers and Upload Schema 3](iteration-0.26.0-upload-parameters-evidence.md), [0.25.0 T16 production frontend and local G6 acceptance](iteration-0.25.0-t16-frontend-evidence.md), [0.24.4 upload data lifecycle and offline delivery](iteration-0.24.4-upload-data-lifecycle-evidence.md), [0.24.3 final source review](iteration-0.24.3-final-review.md), [0.24.2 download/upload integration](iteration-0.24.2-integration-evidence.md), [0.24.1 inline QR login](iteration-0.24.1-qr-login-evidence.md) and [0.24.0 first-platform uploader](iteration-0.24.0-upload-evidence.md).
The managed-file read record centralizes matching binary open, repeated handle identity checks and SHA-256 reading with an exact one-byte growth sentinel. Its read matrix is **27/27 PASS**, the preceding identity matrix is **20/20 PASS**, consumer regression is **103 passed**, Range/mutation/cancellation/copy/cover checks are **14 passed**, and the independent response matrix is **13/13 PASS**. The managed-file identity record gives Editing and Upload one four-field signature and one plain-entry `lstat` implementation while preserving their different I/O and unsafe error mappings. The EditingManager boundary record removes the service, worker, lease, cancellation and stop lifecycle from the HTTP module while preserving the API compatibility alias and service-factory seam; its boundary probe is **5/5 PASS**, focused race cases are **3 passed**, and all Editing modules are **73 passed / 3 stale Schema 1 failures**. The verified media response record removes Editing's reverse private dependency on the top-level API while preserving same-handle full/HEAD/Range streaming, manifest revalidation and handle ownership; its independent response matrix is **26/26 PASS**, the focused checked-in cases are **5 passed**, and editing media is **32 passed**. The edit snapshot record centralizes five Workflow observation paths, distinguishes review from active work and retryable failure from uncertain attention, and binds manual confirmation or retry to the currently observed reason before either mutation can run. All 23 Workflow validators, the 21-case side-effect matrix, and focused follow-up probes pass without changing tracked tests. The upload identity record centralizes ordered targets, account bindings, cancellation batches, and current retry-leaf comparison while preserving the legacy 64-root inspection path, the Workflow 30-slot ceiling, transactional cancellation, session checks, and discovery request digests; its focused upload regression is **243 passed** and the cancellation, concurrency, discovery, restart, multisegment, and synthetic three-platform validators pass. The no-AI whole-video record covers real bundled-FFmpeg output, cover-only output, direct-service and manager restart consistency, LocalAdapter selection, preserved AI whole-video behavior, and production editing/workflow Chrome submission with no external requests. Its focused validator is **5/5 PASS**; the two browser validators and the existing synthetic three-platform AI full-video chain also pass. The hosted CI record binds Actions run 34393235622 to bdd88ce: all four Windows/Linux and CPython 3.12.10/3.13.14 cells passed the environment and pre-test gates, then each failed at the complete pytest stage; the matching local run was 173 failed, 2277 passed and 8 skipped. No tracked tests or product bypasses were added, so this is execution-chain recovery rather than a green CI claim. The download HTTP record covers loopback authority/origin checks, a process-local session token, all eight download-domain write routes, cross-domain token isolation, response headers, session-failure UI and a real local Chrome pass with zero external requests. The preceding source-title record covers Workflow Schema 3 source-title freezing, source-title preset round-trip/materialization, per-platform 80/30/100 title limits, immutable asset binding, metadata-only reconciliation and safe last-preset restoration. Its direct Python validator is **7/7 PASS** and its browser validator is **PASS**; editorial-glass **44/44**, multisegment/readiness/preset browser, full-video/full-chain, upload-attention **7/7**, cancellation, multisegment-service and 100 targeted graph/repository/route checks also passed. The completed UI/upload group is **187 passed, 3 failed**; all three are stale `test_ui_design_system` navigation assertions that omit `/workflows`. Restart-continuation failed once in the parallel group and passed in isolation, so both observations remain recorded. No real URL, OpenAI or target-platform request was made. These records do not form a new frozen release receipt and do not prove real OpenAI responses, human listening quality, real platform acceptance, review, scheduled publication or public visibility. The earlier `0592b6f` receipt remains limited to its own clean frozen commit. Historical evidence remains bound to its recorded build and environment and does not upgrade the current source or an entire platform to verified status. GitHub hosted checks have now run and failed for separately recorded infrastructure and test-contract reasons; required checks/branch protection remain **NOT CONFIGURED**.
