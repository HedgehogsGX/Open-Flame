...
Current iteration: [0.26.0 upload parameters, managed covers and Upload Schema 3](iteration-0.26.0-upload-parameters-evidence.md), following the historical [0.25.0 T16 production frontend and local G6 acceptance](iteration-0.25.0-t16-frontend-evidence.md), [0.24.4 upload data lifecycle and offline delivery](iteration-0.24.4-upload-data-lifecycle-evidence.md), [0.24.3 final source review](iteration-0.24.3-final-review.md), [0.24.2 download/upload integration](iteration-0.24.2-integration-evidence.md), [0.24.1 inline QR login](iteration-0.24.1-qr-login-evidence.md) and [0.24.0 first-platform uploader](iteration-0.24.0-upload-evidence.md).
The 0.26.0 record covers per-target metadata, managed cover validation/preview/deletion, platform scheduling and platform-specific options, Upload Schema 3 and upload backup format 2. It records current local engineering tests but deliberately does not embed its own final Git commit, product identity or artifact hashes; an external release receipt binds the clean detached commit, five artifacts and independent installation after the source record is frozen. GitHub hosted checks are NOT RUN and required checks/branch protection are NOT CONFIGURED. This iteration performs no real login, scan, download, media upload, scheduled publication or platform review. Earlier platform evidence remains bound to its own historical build and does not make 0.26.0 a real-platform acceptance result.
