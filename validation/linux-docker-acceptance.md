...
present. The product contract remains zero to four optional platform sources;
normal operation does not require Cookies. For acceptance only, the operator
enables four non-secret synthetic sources with distinct `acceptance-*` opaque
...
Direct per-file worker mounts remain part of the zero-to-four product contract,
...
5. Keep the host control plane stopped for this isolated run and keep
   ...
   graph Jobs.
6. Configure four non-secret, non-empty synthetic Cookie fixtures for X,
   YouTube, Bilibili, and Douyin. Use distinct `acceptance-*` opaque refs and
