...
- `waiting` and `review` retain their reason and return to
  `awaiting_ai_review`. The later UploadSnapshot slice also made the shared
  classifier boundary reject non-string status/reason values as invalid domain
  data instead of treating an absent reason as an automatic-continuation code.
