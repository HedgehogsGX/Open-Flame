...
class UploadService:
    ...
    def inspect_managed_cover_import(
        ...
        if suffix not in COVER_MIME_TYPES:
        ...
        mime_type, width, height = cover_metadata(payload, suffix)
        return {
            "id": asset_id,
            "name": name,
            "size": len(payload),
        ...
        with self._managed_import("cover", managed_id):
            ...
            if target.exists():
                ...
                try:
                    ...
                    target_metadata = cover_metadata(target_payload, suffix)
                except (OSError, UploadError):
                    raise UploadError("cover_import_conflict") from None
                if (
    ...
    _validate_attempt_state = staticmethod(validate_upload_attempt_state)
