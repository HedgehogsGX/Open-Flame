...
async function mutate(action){if(busy)return;busy=true;document.querySelectorAll('button').forEach(item=>item.disabled=true);
  ...
  finally{busy=false;document.querySelectorAll('button').forEach(item=>item.disabled=false);renderAccounts();renderJobs();renderLogin();if(jobToFocus){const target=[...$('jobs').children].find(item=>item.dataset.jobId===jobToFocus);jobToFocus=null;if(target){target.scrollIntoView({block:'nearest'});target.focus();}}}}
...
function renderJobs(){for(const preview of $('jobs').querySelectorAll('details[data-job-id]'))jobDetailsOpen.set(preview.dataset.jobId,preview.open);$('jobs').replaceChildren();if(!snapshot.jobs.length){$('jobs').append(element('p','尚无上传任务。','muted'));return;}
  for(const job of snapshot.jobs){const item=element('article',undefined,'item');item.dataset.jobId=job.id;item.tabIndex=-1;const source=snapshot.sources.find(entry=>entry.id===job.source_id);item.append(element('strong',platformNames[job.platform]+' · '+job.account_name+' · '+(stateNames[job.state]||job.state)));item.append(element('p',job.title));
