...
from .web_rules import UPLOAD_FORM_RULES_JS
...
<body class="upload-page">
  ...
  <main class="of-shell">
    ...
    <section class="card" aria-labelledby="source-heading">
      ...
      <div id="download-cover-import" class="notice linked-cover-import" hidden>
        <div><p class="linked-cover-title">来源封面（平台返回）</p><p class="muted small">只有点击下方按钮后，才会核验并复制到上传封面库；不会自动创建草稿或触发上传。</p></div>
        <button id="import-download-cover" type="button">导入此来源封面</button>
      </div>
    ...
    <section class="card" aria-labelledby="jobs-heading">
      ...
      <p class="notice small">若结果不确定，请打开本次尝试回执并在平台后台核对，再记录“未接收”“已确认投稿”或“已保存草稿”。只有记录未接收后才能重新创建草稿。取消不能撤回平台已经接收的内容。</p>
      <div id="jobs"></div>
...
const stateNames={draft:'本地草稿',queued:'等待执行',running:'执行中',submitted:'已确认投稿',draft_saved:'已确认草稿已保存',failed:'失败',canceled:'已取消',unknown:'结果不确定',succeeded:'完成',ready:'可用',invalid:'登录失效',unchecked:'尚未检查',checking:'检查中',disconnected:'已断开本地账号'};
...
let draftSubmission=null, sourcesInitialized=false, jobToFocus=null;const jobDetailsOpen=new Map(),uploadAttempts=new Map(),missingUploadAttempts=new Set(),reconciliationAcknowledgements=new Set();
...
Object.assign(codeNames,{cover_not_found:'封面不存在，请重新选择',cover_unavailable:'封面不可用，请重新导入',unsupported_cover_type:'只支持 JPEG、PNG 和 WebP 封面',cover_too_large:'封面超过 20 MiB',cover_empty:'封面文件为空',publish_time_too_soon:'定时发布时间未达到平台提前量',publish_time_invalid:'定时发布时间无效',invalid_publish_time:'定时发布时间无效',scheduled_draft_unsupported:'视频号保存平台草稿时不能设置定时发布',draft_schedule_unsupported:'视频号保存平台草稿时不能设置定时发布',portrait_cover_unsupported:'该平台不支持竖版封面槽位',multiple_covers_unsupported:'抖音一次只能设置一张横版或竖版封面',bilibili_cover_orientation_invalid:'Bilibili 封面需要横版图片',douyin_cover_orientation_invalid:'抖音封面方向与所选横版或竖版槽位不一致',tencent_cover_ratio_invalid:'视频号封面比例不符合对应的 4:3 或 3:4 槽位',unsupported_declaration:'不支持此抖音内容声明',unsupported_content_label:'不支持此视频号内容标记',tencent_short_title_length:'视频号短标题需为 7–15 字',invalid_platform_options:'平台投稿参数无效',invalid_platform_option:'平台投稿参数无效',unsupported_platform_option:'平台投稿参数不受支持',unsupported_platform_field:'该投稿字段不适用于所选平台'});
Object.assign(codeNames,{download_thumbnail_not_found:'来源封面记录不存在，请返回下载页刷新成品',download_thumbnail_unavailable:'来源封面已缺失或校验失败，请返回下载页重新下载',cover_hash_mismatch:'来源封面内容与下载登记不一致，请返回下载页重新下载',cover_size_invalid:'来源封面为空或超过 20 MiB，不能导入上传封面库'});
Object.assign(codeNames,{upload_attempt_not_found:'没有找到这次上传尝试的本地回执',attempt_receipt_missing:'旧任务缺少上传尝试回执，不能安全重传',upload_dispatch_not_started:'应用在调用上传引擎前中断，本次没有开始平台上传，可以重新创建草稿',upload_attempt_identity_changed:'上传尝试的本地身份已变化，请停止操作并检查数据库',upload_attempt_conflict:'上传尝试已被其他操作更新，请刷新后核对',upload_attempt_not_reconcilable:'这次上传尝试当前不能人工核对',reconciliation_acknowledgement_required:'请先确认已在对应平台后台核对',reconciliation_conclusion_invalid:'该结论不适用于此平台动作',manual_remote_not_accepted:'已人工核对：平台未接收本次上传',manual_submission_acknowledged:'已人工核对：平台已接收投稿；审核、公开和定时执行仍未知',manual_platform_draft_saved:'已人工核对：视频号平台草稿已保存'});
...
const attemptStateNames={reserved:'已保留，尚未调用上传引擎',dispatch_may_have_started:'平台调用可能已开始',responded:'上传引擎已返回',unknown:'结果不确定，等待人工核对',reconciled:'已完成人工核对'};
const evidenceKindNames={process_exit_zero:'biliup 进程以 0 退出',uploader_returned_after_final_action:'上传器在最终动作后返回',https_errcode_zero:'投稿 HTTPS 响应 errCode 为 0',post_list_navigation:'保存草稿后进入作品列表'};
const reconciliationNames={not_accepted:'平台未接收',submission_acknowledged:'平台已确认投稿',draft_saved:'视频号草稿已保存'};
function shortDigest(value){return typeof value==='string'&&value.length>=12?value.slice(0,12)+'…':value||'未记录';}
function attemptSummary(attempt){const lines=['本地回执：'+attempt.id,'状态：'+(attemptStateNames[attempt.state]||attempt.state),'创建时间：'+disconnectedTime(attempt.created_at),'请求摘要：'+shortDigest(attempt.request_digest),'任务摘要：'+shortDigest(attempt.job_digest),'来源摘要：'+shortDigest(attempt.source_sha256),'账号会话版本：'+shortDigest(attempt.session_revision),'适配器：'+attempt.adapter_name+' · '+attempt.adapter_revision];if(attempt.dispatch_started_at)lines.push('可能开始平台调用：'+disconnectedTime(attempt.dispatch_started_at));if(attempt.result_code)lines.push('本地结果代码：'+codeText(attempt.result_code));if(attempt.evidence_kind)lines.push('本地证据：'+(evidenceKindNames[attempt.evidence_kind]||attempt.evidence_kind));if(attempt.reconciliation)lines.push('人工结论：'+(reconciliationNames[attempt.reconciliation]||attempt.reconciliation));return lines.join('\n');}
function attemptReceipt(attempt){const receipt=element('section',undefined,'notice');receipt.append(element('strong','本次上传尝试回执'),element('p',attemptSummary(attempt),'details'));return receipt;}
function attemptAllowsRetry(attempt){return !!attempt&&(attempt.state==='responded'&&['failed','canceled'].includes(attempt.result_status)||attempt.state==='reconciled'&&attempt.reconciliation==='not_accepted');}
async function loadUploadAttempt(job){try{const attempt=await api('/jobs/'+encodeURIComponent(job.id)+'/attempt');uploadAttempts.set(job.id,attempt);missingUploadAttempts.delete(job.id);requestedFocusKey='job:'+job.id+(job.state==='unknown'?':reconcile-check':':retry');message(job.state==='unknown'?'已读取本次上传尝试的本地回执；请到 '+platformNames[job.platform]+' 后台核对。':'已读取本次上传尝试的本地回执；请核对后决定是否重新创建草稿。');}catch(error){if(error.code==='upload_attempt_not_found'){uploadAttempts.delete(job.id);missingUploadAttempts.add(job.id);requestedFocusKey='job:'+job.id+':card';message('此任务没有可验证的上传尝试回执，不能安全重传。请从受管视频重新创建本地草稿。',true);return;}throw error;}}
function captureReconciliation(jobId){if(!reconciliationAcknowledgements.has(jobId))throw new Error('请先在对应平台后台核对，并勾选确认。');const attempt=uploadAttempts.get(jobId);if(!attempt)throw new Error('请先读取本次上传尝试回执。');return {attempt_id:attempt.id,expected_revision:attempt.revision};}
async function reconcileUploadAttempt(job,conclusion,captured){const result=await api('/jobs/'+encodeURIComponent(job.id)+'/reconcile',{method:'POST',json:{...captured,conclusion,acknowledge_platform_check:true}});uploadAttempts.set(job.id,result.attempt);reconciliationAcknowledgements.delete(job.id);pinJob(result.job);snapshot.jobs=applyUpdates(snapshot.jobs,[result.job]);jobToFocus=result.job.id;if(conclusion==='not_accepted')message('已记录平台未接收本次上传；现在可以重新创建本地草稿。');else if(conclusion==='draft_saved')message('已记录视频号平台草稿已保存；请继续在平台后台核对草稿内容。');else message('已记录平台已接收投稿；审核、公开状态与定时执行仍需在平台后台核对。');}
function syncReconciliationButtons(jobId=null){for(const item of document.querySelectorAll('button'))if(item.dataset.reconcileJob&&(jobId===null||item.dataset.reconcileJob===jobId))item.disabled=busy||!reconciliationAcknowledgements.has(item.dataset.reconcileJob);}
function message(text,error=false,source='action'){const node=$('message');node.textContent=text;node.className=error?'danger':'muted';node.dataset.messageSource=source;}
...
function renderStorage(){const storage=snapshot.storage;if(!storage){$('storage-summary').textContent='媒体占用暂不可用。';$('storage-warning').textContent='';$('storage-warning').hidden=true;return;}
  const count=value=>Math.max(0,Number(value)||0),parts=['受管副本 '+formatBytes(storage.managed_bytes),'可用空间 '+formatBytes(storage.free_bytes),'写入后预留 '+formatBytes(storage.reserve_bytes),count(storage.registered_source_count)+' 份记录',count(storage.present_source_count)+' 份可用',count(storage.missing_source_count)+' 份缺失',count(storage.deleted_source_count)+' 份已删除'];if(count(storage.changed_source_count))parts.push(count(storage.changed_source_count)+' 份内容变化');if(count(storage.unsafe_source_count))parts.push(count(storage.unsafe_source_count)+' 份路径不安全');if(count(storage.orphan_file_count))parts.push(count(storage.orphan_file_count)+' 份未登记文件（'+formatBytes(storage.orphan_bytes)+'）');$('storage-summary').textContent=parts.join(' · ');
  const warnings=[];if(storage.low_space)warnings.push('本地可用空间仅 '+formatBytes(storage.free_bytes)+'；媒体写入后还需保留 '+formatBytes(storage.reserve_bytes)+'。请先释放空间。');if(count(storage.changed_source_count)||count(storage.unsafe_source_count)||count(storage.orphan_file_count)||count(storage.unsafe_entry_count))warnings.push('存储审计发现异常或未登记条目；这里只读显示，不会自动清理，请检查上传媒体目录。');const warning=warnings.join(' ');if($('storage-warning').textContent!==warning)$('storage-warning').textContent=warning;$('storage-warning').hidden=!warning;}
async function mutate(action){if(busy)return;busy=true;document.querySelectorAll('button').forEach(item=>item.disabled=true);
  ...
  finally{busy=false;document.querySelectorAll('button').forEach(item=>item.disabled=false);renderAccounts();renderSources();renderJobs();renderLogin();syncReconciliationButtons();if(jobToFocus){let target=[...$('jobs').children].find(item=>item.dataset.jobId===jobToFocus);if(!target){try{const record=await api('/jobs/'+encodeURIComponent(jobToFocus));pinJob(record);snapshot.jobs=applyUpdates(snapshot.jobs,[record]);if(!snapshot.sources.some(item=>item.id===record.source_id)){const source=await api('/sources/'+encodeURIComponent(record.source_id));pinSource(source);snapshot.sources=applyUpdates(snapshot.sources,[source]);renderSources();}renderJobs();syncReconciliationButtons();target=[...$('jobs').children].find(item=>item.dataset.jobId===jobToFocus);}catch{}}jobToFocus=null;if(target){target.scrollIntoView({block:'nearest'});target.focus();}}}}
...
function jobScheduleReady(job){if(!job.publish_at_unix)return true;return Number(job.publish_at_unix)*1000>Date.now()+platformScheduleLead(job.platform)*1000;}
  ...
  for(const job of snapshot.jobs){
    ...
    jobSpecs.push({key:'job:'+job.id,signature,build:()=>{
      ...
      if(job.state==='unknown'){
        if(!attempt){if(attemptMissing)item.append(element('p','此任务没有可验证的上传尝试回执，不能人工判定或安全重传。请从上方受管视频重新创建本地草稿。','notice small'));else{const load=focusControl(focus,button('读取本次尝试回执',()=>loadUploadAttempt(job)),'job:'+job.id+':attempt');load.className='secondary';actions.append(load);item.append(element('p','结果不确定时不会直接重传。先读取本地回执，再到平台后台核对本次尝试。','notice small'));}}
        else{item.append(attemptReceipt(attempt));if(attempt.state==='unknown'){const label=element('label',undefined,'choice'),check=focusControl(focus,element('input'),'job:'+job.id+':reconcile-check');check.type='checkbox';check.checked=acknowledged;check.addEventListener('change',()=>{if(check.checked)reconciliationAcknowledgements.add(job.id);else reconciliationAcknowledgements.delete(job.id);syncReconciliationButtons(job.id);});label.append(check,element('span','我已在对应平台后台核对这次上传尝试'));item.append(label);const addConclusion=(text,conclusion)=>{const control=focusControl(focus,button(text,captured=>reconcileUploadAttempt(job,conclusion,captured),()=>captureReconciliation(job.id)),'job:'+job.id+':reconcile-'+conclusion);control.dataset.reconcileJob=job.id;control.disabled=busy||!acknowledged;actions.append(control);};addConclusion('平台未接收','not_accepted');if(job.mode==='publish')addConclusion('平台已确认投稿','submission_acknowledged');if(job.platform==='tencent'&&job.mode==='draft')addConclusion('视频号草稿已保存','draft_saved');}else item.append(element('p','回执状态已经变化，请刷新任务后继续。','notice small'));}
      }
      if(['failed','canceled'].includes(job.state)){if(attempt&&attemptAllowsRetry(attempt)){item.append(attemptReceipt(attempt));if(actionReady)actions.append(focusControl(focus,button('重新创建本地草稿',async()=>{const successor=await api('/jobs/'+encodeURIComponent(job.id)+'/retry',{method:'POST',json:{}});jobToFocus=successor.id;message(successor.state==='draft'?'本地草稿已就绪，已定位到该任务。请核对后再确认。':'未创建新草稿：已有后继任务为“'+(stateNames[successor.state]||successor.state)+'”。已定位到该任务，请查看其当前状态。');}),'job:'+job.id+':retry'));}else if(attemptMissing)item.append(element('p','此任务没有可验证的上传尝试回执，不能安全重传。请从上方受管视频重新创建本地草稿。','notice small'));else if(!attempt){const load=focusControl(focus,button('读取回执并检查重试资格',()=>loadUploadAttempt(job)),'job:'+job.id+':attempt');load.className='secondary';actions.append(load);item.append(element('p','只有读取并验证到可重试的本地回执后，才会提供重新创建草稿操作。','notice small'));}else{item.append(attemptReceipt(attempt));item.append(element('p','此回执状态不允许重试；请刷新并按当前状态处理。','notice small'));}}
      if(actions.children.length)item.append(actions);return item;
    }});
  }
  reconcileKeyed($('jobs'),jobSpecs,focus);syncReconciliationButtons();restoreRenderedFocus(focus);
}
...
async function poll(){if(timer!==null)clearTimeout(timer);timer=null;if(!pageVisible()){pollingSuspended=true;return;}try{if(!busy)await refresh();}catch(error){message('状态刷新失败：'+error.message,true,'poll');}finally{schedulePoll();}}
...
function platformScheduleLead(platform){return uploadScheduleLead(platform,snapshot.status?.platforms?.find(item=>item.id===platform)?.schedule_min_lead_seconds);}
function renderScheduleLabels(){for(const platform of Object.keys(platformNames)){$(platform+'-publish-at').previousElementSibling.textContent='定时发布（至少提前 '+leadText(platformScheduleLead(platform))+(platform==='tencent'?'，整点，最多 28 天':'')+'）';}}
function scheduleValue(platform){
  const controlId=platform+'-publish-at',control=$(controlId),lead=platformScheduleLead(platform);
  const result=localPublishSchedule(platform,control.value,Date.now(),control.required,lead);
  if(!result.error)return result.schedule;
  const messages={required:'请选择固定的发布日期和时间。',invalid:'定时发布时间无效，可能落在本地夏令时跳时区间。',ambiguous:'该本地时间在夏令时切换时出现两次，请选择其他时间。',lead:platformNames[platform]+' 定时发布至少需要提前 '+leadText(lead)+'。',hour:'视频号定时发布只支持整点。',horizon:'视频号定时发布最多可提前 28 天。'};
  return {error:messages[result.error],controlId};
}
...
const importParams=new URL(location.href).searchParams;
const assetId=importParams.get('asset_id');if(assetId&&/^[0-9a-f-]{36}$/.test(assetId)){$('asset-import').hidden=false;$('import-asset').addEventListener('click',()=>mutate(async()=>{const source=await api('/sources/assets/'+encodeURIComponent(assetId),{method:'POST'});await refresh();pinSource(source);snapshot.sources=applyUpdates(snapshot.sources,[source]);renderSources();$('source-id').value=source.id;showSource();$('asset-import').hidden=true;message('下载成品已导入，原下载文件保持不变。');}));}
const coverArtifactId=importParams.get('cover_artifact_id');if(coverArtifactId&&/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(coverArtifactId)){$('download-cover-import').hidden=false;$('import-download-cover').addEventListener('click',()=>mutate(async()=>{message('正在核验并导入来源封面…');const cover=await api('/covers/download-artifacts/'+encodeURIComponent(coverArtifactId),{method:'POST'});snapshot.covers=applyUpdates(snapshot.covers,[cover]);requestedFocusKey='cover:'+cover.id+':card';renderCovers();$('download-cover-import').hidden=true;const nextUrl=new URL(location.href);nextUrl.searchParams.delete('cover_artifact_id');history.replaceState(history.state,'',nextUrl);message('来源封面已导入受管封面库；请在对应平台的封面选项中明确选择。尚未创建草稿或上传。');}));}
...
(async()=>{try{await refresh();message('选择账号和视频，创建本地草稿后确认执行。');}catch(error){message('上传器连接失败：'+error.message,true,'poll');}finally{schedulePoll();}})();
</script></body></html>'''
