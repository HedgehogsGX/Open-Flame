  ...
  <main class="of-shell">
    ...
    <section class="card" aria-labelledby="compose-heading">
      ...
      <form id="job-form" aria-labelledby="compose-heading">
        ...
        <fieldset id="bilibili-options" hidden><legend>Bilibili 投稿信息</legend><label for="category-id">分区 ID（tid）</label><input id="category-id" type="number" min="1" step="1" placeholder="填写目标分区 ID" aria-describedby="job-form-error"><label for="copyright">投稿类型</label><select id="copyright" aria-describedby="job-form-error"><option value="">请选择原创或转载</option><option value="1">原创</option><option value="2">转载</option></select><label for="source-credit">转载来源</label><input id="source-credit" maxlength="200" placeholder="转载时必须填写来源" aria-describedby="job-form-error"></fieldset>
        <p class="muted">Bilibili、抖音确认后立即投稿；视频号按上方所选动作执行。此按钮只创建本地草稿，不上传视频。</p>
...
let csrf='', snapshot={accounts:[],sources:[],jobs:[],operations:[],status:{},storage:null}, busy=false, pollPromise=null, timer=null, pageActive=true, pollingSuspended=!!document.hidden;
...
let sourceNextCursor=null,jobNextCursor=null,sourceHistoryExpanded=false,jobHistoryExpanded=false;
...
const accountDisconnectOpen=new Set(),sourceDeleteOpen=new Set();let requestedFocusKey=null,inputComposing=false;
...
function renderJobs(){
  ...
  for(const job of snapshot.jobs){
    const source=snapshot.sources.find(entry=>entry.id===job.source_id),mediaState=jobMediaState(job,source),mediaReady=mediaState==='present'&&job.source_media_present!==false,accountReady=jobAccountActive(job),actionReady=mediaReady&&accountReady;
    const signature=[job.platform,job.account_id,job.account_name,job.account_lifecycle_state,job.source_id,job.source_name,job.source_media_state,job.source_media_present,job.title,job.description,job.tags||[],job.mode,job.category_id,job.copyright,job.source_credit,job.state,job.code,source?.name||null,source?sourceMediaState(source):null,source?.media_present,mediaState,mediaReady,accountReady];
    jobSpecs.push({key:'job:'+job.id,signature,build:()=>{
      ...
      if(!mediaReady)item.append(element('p','此任务的受管副本'+(mediaStateNames[mediaState]||'不可用')+'；请先恢复受管副本：在上方媒体记录中重新导入相同视频，之后再确认或重新创建本地草稿。','notice small'));if(!accountReady)item.append(element('p','此任务的账号已断开，不能再确认或重试。请连接新账号并创建新的本地草稿。','notice small'));
      const actions=element('div',undefined,'row');if(job.state==='draft'&&actionReady)actions.append(focusControl(focus,button(job.mode==='draft'?'确认上传并保存平台草稿':'确认立即上传投稿',async()=>{const updated=await api('/jobs/'+encodeURIComponent(job.id)+'/confirm',{method:'POST'});pinJob(updated);message('已确认此任务，等待上传器执行。');}),'job:'+job.id+':confirm'));
...
async function refresh(){if(pollPromise)return pollPromise;pollPromise=(async()=>{const volatile=[];for(const item of [...pinnedJobs.values(),...snapshot.jobs])if(['queued','running'].includes(item.state)&&!volatile.includes(item.id)&&volatile.length<64)volatile.push(item.id);const status=await api('/status');snapshot.status=status;renderEngineStatus(status);const results=await Promise.allSettled([api('/accounts'),api('/sources/page?limit=200'),api('/jobs/page?limit=200'),api('/operations'),api('/storage')]);let {accounts,sources,jobs,operations,storage}=snapshot,failed=results.some(item=>item.status==='rejected');if(results[0].status==='fulfilled')accounts=results[0].value;if(results[1].status==='fulfilled'){const sourcePage=results[1].value;for(const item of sourcePage.items)if(pinnedSources.has(item.id))pinSource(item);sources=mergeFirstPage(sourcePage.items,snapshot.sources,sourceHistoryExpanded);if(!sourceHistoryExpanded)sourceNextCursor=sourcePage.next_cursor;}sources=applyUpdates(sources,[...pinnedSources.values()]);if(results[2].status==='fulfilled'){const jobPage=results[2].value,firstIds=new Set(jobPage.items.map(item=>item.id)),missing=volatile.filter(id=>!firstIds.has(id));let resolved=[];if(missing.length){try{resolved=await api('/jobs/resolve?ids='+encodeURIComponent(missing.join(',')));}catch{failed=true;}}for(const item of jobPage.items)if(pinnedJobs.has(item.id))pinJob(item);for(const item of resolved)pinJob(item);jobs=mergeFirstPage(jobPage.items,snapshot.jobs,jobHistoryExpanded||failed);if(!jobHistoryExpanded)jobNextCursor=jobPage.next_cursor;}jobs=applyUpdates(jobs,[...pinnedJobs.values()]);if(results[3].status==='fulfilled')operations=results[3].value;if(results[4].status==='fulfilled')storage=results[4].value;snapshot={status,accounts,sources,jobs,operations,storage};$('records-status').hidden=!failed;$('records-status').textContent=failed?'账号、视频、任务或媒体占用更新失败；当前保留上次成功读取的内容。恢复本地存储后请刷新。':'';renderAccounts();renderSources();renderJobs();renderStorage();renderLogin();})();try{await pollPromise;}finally{pollPromise=null;}}
...
for(const id of ['account-name','title','description','tags','source-credit']){const input=$(id);input.addEventListener('compositionstart',()=>{inputComposing=true;});input.addEventListener('compositionend',()=>{inputComposing=false;});input.addEventListener('keydown',event=>{if(event.key==='Enter'&&(event.isComposing||event.keyCode===229||inputComposing))event.preventDefault();});}
function beginSubmit(event){event.preventDefault();return !(event.isComposing||inputComposing);}
const jobValidationControls=['account-selection','source-id','title','category-id','copyright','source-credit'];
...
$('job-form').addEventListener('submit',event=>{if(!beginSubmit(event))return;clearJobValidation();const accountIds=selectedAccounts(),selected=snapshot.accounts.filter(item=>accountIds.includes(item.id)).map(item=>({id:item.id,platform:item.platform})),sourceId=$('source-id').value,title=$('title').value.trim(),description=$('description').value,tags=$('tags').value.split(/[,，]/).map(item=>item.trim()).filter(Boolean),categoryId=$('category-id').value,copyright=$('copyright').value,sourceCredit=$('source-credit').value.trim(),tencentMode=$('tencent-mode').value,bili=selected.some(item=>item.platform==='bilibili');if(!accountIds.length)return rejectJobDraft('account-selection','请选择至少一个账号。');if(!sourceId)return rejectJobDraft('source-id','请先导入并选择视频。');if(!title)return rejectJobDraft('title','请填写标题。');if(bili&&!categoryId)return rejectJobDraft('category-id','请填写 Bilibili 分区 ID。');if(bili&&!copyright)return rejectJobDraft('copyright','请选择 Bilibili 原创或转载。');if(bili&&copyright==='2'&&!sourceCredit)return rejectJobDraft('source-credit','转载必须填写来源。');mutate(async()=>{
  const base={source_id:sourceId,title,description,tags,category_id:bili?Number(categoryId):null,copyright:bili?Number(copyright):1,source_credit:sourceCredit};
  const signature=JSON.stringify({...base,accountIds:[...accountIds].sort(),tencentMode});if(!draftSubmission||draftSubmission.signature!==signature)draftSubmission={signature,key:crypto.randomUUID()};
  const created=[];for(const mode of ['publish','draft']){const group=selected.filter(item=>(item.platform==='tencent'?tencentMode:'publish')===mode).map(item=>item.id);if(group.length){const rows=await api('/jobs',{method:'POST',json:{...base,account_ids:group,mode,idempotency_key:draftSubmission.key+'-'+mode}});if(Array.isArray(rows))created.push(...rows);}}
  if(created.length){for(const item of created)pinJob(item);snapshot.jobs=applyUpdates(snapshot.jobs,created);}draftSubmission=null;message('本地草稿已创建。请在下方核对每份内容，再确认执行。');});});
