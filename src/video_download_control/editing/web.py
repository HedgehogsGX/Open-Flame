...
function renderAiTasks(rows){
  ...
  for(const task of rows){
    ...
    if(task.state==='review'&&ownerBlocked){const cancel=focusControl(element('button','取消被阻止的任务','secondary'),key+'cancel');cancel.type='button';cancel.addEventListener('click',()=>mutate(async()=>{await api('/ai-tasks/'+encodeURIComponent(task.id)+'/cancel',{method:'POST'});message('被账本阻止的待确认任务已取消。');await refreshRecords();}));actions.append(cancel);}
...
function renderPlans(plans){
  ...
  for(const plan of plans){
    ...
    if(state==='review'&&ownerBlocked){const cancel=focusControl(element('button','取消被阻止的计划','secondary'),key+'cancel');cancel.type='button';cancel.addEventListener('click',()=>mutate(async()=>{await api('/plans/'+encodeURIComponent(plan.id)+'/cancel',{method:'POST'});message('被账本阻止的待确认计划已取消。');await refreshRecords();}));actions.append(cancel);}
