...
<body class="editing-page">
  ...
  <main class="of-shell">
    <header class="page-header">
      ...
      <p class="lede">不添加分段时生成完整视频，也可设置多个分段并抽取封面；处理计划保存后仍需单独确认。下载原件不会被覆盖，编辑成品进入上传器时还会再次复制并校验。</p>
    ...
    <section id="workspace" class="card" aria-labelledby="workspace-heading" hidden>
      ...
      <div class="editor-grid">
        ...
        <form id="draft-form" aria-labelledby="workspace-heading">
          ...
          <fieldset>
            <legend>分段</legend>
            <p class="muted small">0 段表示生成一个完整视频；添加分段后只生成所选范围。</p>
...
function readRecipe(){const segments=[...document.querySelectorAll('[data-segment]')].map((row,index)=>({start_ms:Math.round(Number(row.querySelector('[data-start]').value)*1000),end_ms:Math.round(Number(row.querySelector('[data-end]').value)*1000),label:row.querySelector('[data-label]').value.trim()||('片段 '+(index+1))}));for(const [index,segment] of segments.entries())if(!Number.isSafeInteger(segment.start_ms)||!Number.isSafeInteger(segment.end_ms)||segment.start_ms<0||segment.end_ms<=segment.start_ms)throw new Error('分段 '+(index+1)+' 的起止时间无效。');let cover=null;if($('cover-enabled').checked){cover={timestamp_ms:Math.round(Number($('cover-at').value)*1000),aspect_ratio:$('cover-aspect').value,title:$('cover-title').value.trim(),subtitle:''};if(!Number.isSafeInteger(cover.timestamp_ms)||cover.timestamp_ms<0)throw new Error('封面抽帧时间无效。');}let translation=null,dubbing=null;if($('use-translation').checked){const revision=selectedTranslation(),parent=parentTimeline(revision);if(!revision||!parent){if(loadedAiRecipe.translation?.enabled&&!loadedAiRecipe.translation?.revision_id)throw new Error('此旧草稿没有绑定具体译文版本；请在下方重新选择已批准译文，或取消使用译文。');throw new Error('已保存的译文版本当前不可用；请重新选择一份已批准且来源完整的译文。');}translation={enabled:true,source_language:parent.language,target_language:revision.language,provider:revision.provider,model:revision.model,state:'ready',revision_id:revision.id};if($('dubbing-enabled').checked){const capability=selectedCapability('dub'),voice=$('voice-id').value,rawRate=$('voice-rate').value,rate=Number(rawRate);if(!capability||!capability.authorization||!standardVoices(capability).some(item=>item.id===voice))throw new Error('请选择当前 provider 提供的标准音色。');if(!rawRate||!Number.isFinite(rate)||rate<0.88||rate>1.12){$('voice-rate').focus();throw new Error('请选择 0.88×～1.12× 的配音语速。');}dubbing={enabled:true,language:revision.language,provider:capability.provider_id,model:capability.model_id,voice,state:'ready',replace_original_audio:$('mix-mode').value==='replace',authorization:capability.authorization,rate};}}else if($('dubbing-enabled').checked){throw new Error('启用配音前请先选择已批准译文。');}return {segments,cover,translation,dubbing};}
function loadRecipe(recipe){$('segments').replaceChildren();for(const segment of recipe?.segments||[])addSegment(segment);$('cover-enabled').checked=!!recipe?.cover;$('cover-controls').hidden=!recipe?.cover;if(recipe?.cover){$('cover-at').value=String(Number(recipe.cover.timestamp_ms)/1000);if([...$('cover-aspect').options].some(option=>option.value===recipe.cover.aspect_ratio))$('cover-aspect').value=recipe.cover.aspect_ratio;$('cover-title').value=recipe.cover.title||'';}loadedAiRecipe={translation:recipe?.translation||null,dubbing:recipe?.dubbing||null};activeTranslationId=null;$('use-translation').checked=!!recipe?.translation?.enabled;$('dubbing-enabled').checked=!!recipe?.dubbing?.enabled;$('mix-mode').value=recipe?.dubbing?.replace_original_audio?'replace':'mix';$('voice-rate').value=String(recipe?.dubbing?.rate??1);chooseCapabilityForRecipe('translation-capability','translate',loadedAiRecipe.translation);chooseCapabilityForRecipe('voice-provider','dub',loadedAiRecipe.dubbing);syncAiControls();}
...
function renderPlans(plans){
  ...
  for(const plan of plans){
    ...
    const speechCapability=dubbing?.enabled?aiCapabilities.find(row=>row.operation==='dub'&&authorizationMatches(row,plan.dubbing_authorization_sha256)):null,timelineBindingValid=!translation?.enabled||(/^[0-9a-f]{32}$/.test(plan.timeline_revision_id||'')&&/^[0-9a-f]{64}$/.test(plan.timeline_cues_sha256||'')&&(!translation.revision_id||translation.revision_id===plan.timeline_revision_id));
    ...
    if(translation?.enabled)copy.append(element('p','已批准译文 '+translation.source_language+' → '+translation.target_language+' · '+translation.provider+' · '+translation.model,'muted small'),element('p',timelineBindingValid?'译文修订 '+String(plan.timeline_revision_id).slice(0,12)+'… · cues '+String(plan.timeline_cues_sha256).slice(0,16)+'…':'译文修订绑定缺失或不一致','mono small'));
    ...
    head.append(copy);
    if(!timelineBindingValid)item.append(element('p','此计划没有可核对的精确译文修订绑定；请从当前草稿重新生成计划。','notice small'));
    ...
    if(state==='review'&&!ownerBlocked){
      ...
      confirm.type='button';
      confirm.dataset.planTimelineValid=String(timelineBindingValid);
      confirm.disabled=!timelineBindingValid;
      confirm.addEventListener('click',()=>mutate(async()=>{
        if(!timelineBindingValid)throw new Error('此计划没有可核对的精确译文修订绑定，请从当前草稿重新生成计划。');
