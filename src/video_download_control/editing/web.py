    ...
    <section class="card" aria-labelledby="ai-heading">
      ...
      <div class="diagnostic-grid">
        ...
        <div class="diagnostic-panel">
          ...
          <p class="muted small">声音克隆不会出现在这里。批准译文会把 translation 以 ready 状态写入当前草稿；选择配音后请保存草稿。每个待处理计划会再次显示绑定的 provider、模型、音色与数据范围。</p>
        </div>
      </div>
...
let aiCapabilities=[],aiTasks=[],timelines=[],loadedAiRecipe={translation:null,dubbing:null},activeTranslationId=null;
...
function capabilityKey(row){return [row.operation,row.provider_id||'',row.model_id||''].join('|');}
function usableCapabilities(operation){return aiCapabilities.filter(row=>row.operation===operation&&row.status!=='blocked'&&row.provider_id&&row.model_id);}
...
function chooseCapabilityForRecipe(id,operation,recipe){if(!recipe?.enabled)return;const row=usableCapabilities(operation).find(item=>item.provider_id===recipe.provider&&item.model_id===recipe.model);if(row)$(id).value=capabilityKey(row);}
function updateVoiceOptions(){const select=$('voice-id'),previous=select.value,preferred=loadedAiRecipe.dubbing?.voice||'',voices=standardVoices(selectedCapability('dub'));select.replaceChildren();for(const voice of voices){const option=element('option',voice.label||voice.id);option.value=voice.id;select.append(option);}const wanted=[previous,preferred].find(value=>voices.some(voice=>voice.id===value));if(wanted)select.value=wanted;if(!voices.length){const option=element('option','此 provider 未公布标准音色');option.value='';select.append(option);}updateActionState();}
function renderCapabilities(rows){aiCapabilities=rows;const host=$('capabilities');host.replaceChildren();for(const row of rows){const card=element('article',undefined,'capability-card');card.append(element('h3',row.label||operationNames[row.operation]||row.operation));const status=element('p',row.status==='ready'?'可用':row.status==='blocked'?'尚不可用':'完整性已验证，执行健康待确认','status-value');card.append(status,element('p',row.description||row.reason||row.reason_code||'','muted small'));if(row.provider_id)card.append(element('p','Provider：'+row.provider_id+(row.model_id?' · '+row.model_id:''),'mono small'));card.append(element('p',dataScope(row),'muted small'));if(row.operation==='dub'&&standardVoices(row).length)card.append(element('p','标准音色：'+standardVoices(row).map(voice=>voice.label||voice.id).join('、'),'muted small'));host.append(card);}if(!rows.length)host.append(element('p','能力清单暂不可用。','muted'));fillCapabilitySelect('transcription-capability','transcribe');fillCapabilitySelect('translation-capability','translate');fillCapabilitySelect('voice-provider','dub');chooseCapabilityForRecipe('translation-capability','translate',loadedAiRecipe.translation);chooseCapabilityForRecipe('voice-provider','dub',loadedAiRecipe.dubbing);updateVoiceOptions();syncAiControls();}
function renderAiTasks(rows){aiTasks=rows;const host=$('ai-tasks');host.replaceChildren();if(!selectedProject){host.append(element('p','请先打开一个编辑项目。','muted'));return;}if(!rows.length){host.append(element('p','当前项目尚无 AI 任务。','muted'));return;}for(const task of rows){const item=element('article',undefined,'item'),head=element('div',undefined,'row-spread'),copy=element('div'),key='ai-task:'+task.id+':',cap=aiCapabilities.find(row=>row.operation===(task.operation==='translate'?'translate':'transcribe')&&row.provider_id===task.provider&&row.model_id===task.model);copy.append(element('h3',(operationNames[task.operation]||task.operation)+'任务 '+String(task.id).slice(0,8)),element('p',(stateNames[task.state]||task.state)+' · '+task.provider+' · '+task.model,'muted small'));if(task.operation==='translate')copy.append(element('p','来源时间轴 '+String(task.source_revision_id||'').slice(0,8)+' · 目标语言 '+(task.target_language||'未知'),'muted small'));head.append(copy);item.append(head,element('p',dataScope(cap),'notice small'));const actions=element('div',undefined,'row');if(task.state==='review'){const confirm=focusControl(element('button','确认并执行'),key+'confirm');confirm.type='button';confirm.dataset.aiConfirm=capabilityKey({operation:task.operation,provider_id:task.provider,model_id:task.model});confirm.addEventListener('click',()=>mutate(async()=>{await api('/ai-tasks/'+encodeURIComponent(task.id)+'/confirm',{method:'POST'});message('AI 任务已明确确认并进入队列。');await refreshRecords();}));const cancel=focusControl(element('button','取消任务','secondary'),key+'cancel');cancel.type='button';cancel.addEventListener('click',()=>mutate(async()=>{await api('/ai-tasks/'+encodeURIComponent(task.id)+'/cancel',{method:'POST'});message('待确认 AI 任务已取消。');await refreshRecords();}));actions.append(confirm,cancel);}if(['queued','running','canceling'].includes(task.state)){const cancel=focusControl(element('button','请求取消','secondary'),key+'cancel');cancel.type='button';cancel.addEventListener('click',()=>mutate(async()=>{await api('/ai-tasks/'+encodeURIComponent(task.id)+'/cancel',{method:'POST'});message('已请求取消 AI 任务。');await refreshRecords();}));actions.append(cancel);}if(['failed','canceled'].includes(task.state)){item.append(element('p','中断前已完成的远程批次可能已经计费；重试会重新发送整个听写或翻译任务，可能再次计费。','notice small'));const retry=focusControl(element('button','建立待确认重试任务','secondary'),key+'retry');retry.type='button';retry.addEventListener('click',()=>mutate(async()=>{await api('/ai-tasks/'+encodeURIComponent(task.id)+'/retry',{method:'POST',body:JSON.stringify({idempotency_key:randomKey('ai-retry')})});message('已建立待确认重试任务；尚未执行。上次已完成的远程批次可能已计费。');await refreshRecords();}));actions.append(retry);}if(actions.children.length)item.append(actions);if(task.code)item.append(element('p',codeNames[task.code]||task.code,'muted small'));host.append(item);}updateActionState();}
...
async function createTranscriptionTask(){if(!selectedProject)throw new Error('请先打开项目。');const capability=selectedCapability('transcribe');if(!capability)throw new Error('当前没有可执行的听写能力。');const task=await api('/projects/'+encodeURIComponent(projectId(selectedProject))+'/ai-tasks',{method:'POST',body:JSON.stringify({operation:'transcribe',provider:capability.provider_id,model:capability.model_id,options:{language:$('transcription-language').value||null,word_timestamps:true,vad:true},idempotency_key:randomKey('transcribe')})});message('已建立待确认听写任务 '+String(task.id).slice(0,8)+'；尚未执行。');await refreshRecords();}
async function createTranslationTask(source){const capability=selectedCapability('translate'),target=$('target-language').value;if(!capability)throw new Error('当前没有可执行的翻译能力。');if(!target||target===source.language)throw new Error('目标语言必须与字幕语言不同。');const task=await api('/projects/'+encodeURIComponent(projectId(selectedProject))+'/ai-tasks',{method:'POST',body:JSON.stringify({operation:'translate',provider:capability.provider_id,model:capability.model_id,source_revision_id:source.id,options:{source_language:source.language,target_language:target,glossary:[]},idempotency_key:randomKey('translate')})});message('已建立待确认翻译任务 '+String(task.id).slice(0,8)+'；尚未执行。');await refreshRecords();}
function renderPlans(plans){
  const host=$('plans');host.replaceChildren();
  if(!plans.length){host.append(element('p','尚无处理计划。','muted'));return;}
  for(const plan of plans){
    const item=element('article',undefined,'item'),head=element('div',undefined,'row-spread'),copy=element('div'),state=plan.state||'review',key='plan:'+plan.id+':',translation=plan.recipe?.translation,dubbing=plan.recipe?.dubbing;
    const speechCapability=dubbing?.enabled?aiCapabilities.find(row=>row.operation==='dub'&&row.provider_id===dubbing.provider&&row.model_id===dubbing.model):null;
    ...
    if(dubbing?.enabled)copy.append(element('p','标准音色 '+dubbing.voice+' · '+(dubbing.replace_original_audio?'替换原声':'压低原声后叠加')+' · '+dubbing.provider+' · '+dubbing.model,'muted small'));
    head.append(copy);
    const actions=element('div',undefined,'row');
    let egressConsent=null;
    if(state==='review'&&dubbing?.enabled){
      const consent=element('label',undefined,'choice'),input=document.createElement('input'),summary=speechCapability?.execution==='remote'
        ?'我确认此计划会把每个已批准译文 cue 与标准音色参数发送给 '+dubbing.provider+'；真实调用可能产生费用。Provider 清单声明：'+((speechCapability.data_egress||[]).join('、')||'未声明')+'。'
        :'我确认此计划使用 '+dubbing.provider+' · '+dubbing.model+' 生成配音；当前能力清单声明为本地执行。';
    ...
    if(state==='review'){
      ...
      confirm.addEventListener('click',()=>mutate(async()=>{
        ...
        if(dubbing?.enabled&&!egressConsent?.checked)throw new Error('请先核对并确认此计划的配音数据范围与费用。');
        await api('/plans/'+encodeURIComponent(plan.id)+'/confirm',{method:'POST',body:JSON.stringify({expected_recipe_sha256:plan.recipe_sha256,ai_data_egress_accepted:Boolean(dubbing?.enabled&&egressConsent?.checked)})});
...
async function refreshRecords(){if(recordsPromise)return recordsPromise;const capturedGeneration=projectSelectionGeneration,capturedProjectId=currentProjectId(),projectParam=capturedProjectId?'?project_id='+encodeURIComponent(capturedProjectId):'';const work=(async()=>{const [projects,plans,outputs,tasks,revisions]=await Promise.all([api('/projects'),api('/plans'+projectParam),api('/assets'+projectParam),capturedProjectId?api('/ai-tasks'+projectParam):Promise.resolve([]),capturedProjectId?api('/timelines'+projectParam):Promise.resolve([])]);if(capturedGeneration!==projectSelectionGeneration||capturedProjectId!==currentProjectId())return false;renderIfChanged('projects',{selected_project_id:capturedProjectId,items:projects},value=>renderProjects(value.items));renderIfChanged('plans',plans,renderPlans);renderIfChanged('outputs',outputs,renderOutputs);renderIfChanged('ai-tasks',{project_id:capturedProjectId,items:tasks},value=>renderAiTasks(value.items));renderIfChanged('timelines',{project_id:capturedProjectId,items:revisions},value=>renderTimelines(value.items));return true;})();recordsPromise=work;try{return await work;}finally{if(recordsPromise===work)recordsPromise=null;}}
...
$('voice-id').addEventListener('change',markDirty);
$('mix-mode').addEventListener('change',markDirty);
$('use-translation').addEventListener('change',()=>{if(!$('use-translation').checked)$('dubbing-enabled').checked=false;markDirty();updateActionState();});
$('dubbing-enabled').addEventListener('change',()=>{markDirty();updateActionState();});
