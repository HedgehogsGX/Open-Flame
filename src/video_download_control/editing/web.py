...
function chooseCapabilityForRecipe(id,operation,recipe){if(!recipe?.enabled)return;const row=usableCapabilities(operation).find(item=>item.provider_id===recipe.provider&&item.model_id===recipe.model&&(!recipe.authorization||JSON.stringify(item.authorization)===JSON.stringify(recipe.authorization)));if(row)$(id).value=capabilityKey(row);}
...
function renderAiTasks(rows){
  ...
  for(const task of rows){
    const operation=task.operation==='translate'?'translate':'transcribe',item=element('article',undefined,'item'),head=element('div',undefined,'row-spread'),copy=element('div'),key='ai-task:'+task.id+':',cap=aiCapabilities.find(row=>row.operation===operation&&authorizationMatches(row,task.authorization_sha256));
    ...
    if(task.state==='review'&&task.authorization?.execution==='remote'){
...
function renderPlans(plans){
  ...
  for(const plan of plans){
    ...
    if(dubbing?.enabled)copy.append(element('p','标准音色 '+dubbing.voice+' · '+(dubbing.replace_original_audio?'替换原声':'压低原声后叠加')+' · '+dubbing.provider+' · '+dubbing.model,'muted small'),element('p',authorizationSummary(dubbing.authorization),'mono small'));
    head.append(copy);
    ...
    if(state==='review'&&dubbing?.enabled){
      const consent=element('label',undefined,'choice'),input=document.createElement('input'),summary=dubbing.authorization?.execution==='remote'
        ?'我确认此计划会把每个已批准译文 cue 与标准音色参数发送给 '+dubbing.provider+'；真实调用可能产生费用。冻结范围：'+((dubbing.authorization.data_egress||[]).join('、')||'未声明')+'。'
