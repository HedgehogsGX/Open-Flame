...
from .assets import CAPTION_MIME_TYPES, THUMBNAIL_MIME_TYPES
...
from .download_assets import (
    DownloadAssetReadError,
    DownloadAssetReader,
    DownloadReadCancelled,
    canonical_download_asset_id,
    read_original_snapshot,
    read_registered_auxiliary,
    resolve_registered_original,
)
...
from .verified_media_response import (
    VERIFIED_STREAM_CHUNK_BYTES as _VERIFIED_STREAM_CHUNK_BYTES,
    VerifiedOpenFileResponse as _VerifiedOriginalFileResponse,
    close_binary_handle as _close_binary_handle,
)
from .schemas import (
    BatchAssetResponse,
    BatchCreateRequest,
...
def _public_auxiliary_artifact(record: object) -> dict[str, str | None] | None:
    ...
    if not isinstance(record, dict):
        return None
    try:
        artifact_id = canonical_download_asset_id(record.get("artifact_id"))
    except ValueError:
        return None
...
def create_app(
    ...
    *,
    ...
    download_reader = DownloadAssetReader(resolved_settings.data_root, service)
    app.state.download_asset_reader = download_reader

    def upload_original_asset(asset_id: str) -> tuple[Path, str]:
        try:
            return download_reader.resolve_video_original(asset_id)
        except DownloadAssetReadError as error:
            code = 404 if error.code == "asset_not_found" else 409
            raise HTTPException(status_code=code, detail=error.code) from None

    def upload_download_thumbnail(artifact_id: str) -> tuple[Path, str, str]:
        try:
            return download_reader.resolve_thumbnail(artifact_id)
        except DownloadAssetReadError as error:
            code = 404 if error.code == "download_thumbnail_not_found" else 409
            raise HTTPException(status_code=code, detail=error.code) from None

    def workflow_download_caption(artifact_id: str) -> dict[str, object]:
        return download_reader.read_caption(artifact_id, maximum_bytes=MAX_SUBTITLE_BYTES)

    def workflow_download_cover(asset_id: str) -> Mapping[str, object] | None:
        try:
            return download_reader.unique_thumbnail(asset_id)
        except DownloadAssetReadError as error:
            raise WorkflowError("workflow_" + error.code) from None
    ...
    def list_batch_assets(batch_id: str) -> list[BatchAssetResponse]:
        ...
        for record in records:
            asset_id = canonical_download_asset_id(record["asset_id"])
            public_artifacts = [
                public
    ...
    def download_asset(asset_id: str) -> Response:
        ...
        try:
            path, file_info = resolve_registered_original(
    ...
    def download_auxiliary_artifact(artifact_id: str) -> StreamingResponse:
        ...
        try:
            payload, size_bytes, mime_type, suffix = read_registered_auxiliary(
                resolved_settings.data_root,
