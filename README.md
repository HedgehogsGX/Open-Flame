...
| 自动流程 | `/workflows`；页面可输入或从预设恢复 1–10 个有序分段，持久化串接 URL 下载、AI 审核/确认，以及每段向最多 3 个账号建立上传草稿；最多 30 个任务在一次批量确认中共同通过或共同停止。逐段准备以稳定幂等键和 prefix checkpoint 恢复；账号登录 revision 改变时拒绝旧绑定，重试 leaf 只在原 segment/source/account/platform 位置更新，未知远端结果停下人工核对 |
...
准确的当前运维步骤与边界见 [Runbook](docs/RUNBOOK.md)。本轮记录在[多分段自动流程记录](validation/iteration-0.28.0-multisegment-workflow.md)与 [Iteration 0.28.0 AI 与自动流程证据](validation/iteration-0.28.0-ai-workflow-evidence.md)；[Iteration 0.27.0 编辑工作台证据](validation/iteration-0.27.0-editing-workspace-evidence.md)及更早记录均为 point-in-time 历史证据，不替代当前构建、AI 模型或真实平台验收。
