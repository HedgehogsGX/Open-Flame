...
| 自动流程 | `/workflows`；页面可输入或从预设恢复 1–10 个有序分段，并在创建前分别显示下载 Worker、AI runtime/凭据、上传 runtime/调度器和所选账号的即时状态；持久化串接 URL 下载、AI 审核/确认，以及每段向最多 3 个账号建立上传草稿；最多 30 个任务在一次批量确认中共同通过或共同停止。逐段准备以稳定幂等键和 prefix checkpoint 恢复；账号登录 revision 改变时拒绝旧绑定，重试 leaf 只在原 segment/source/account/platform 位置更新，未知远端结果停下人工核对 |
